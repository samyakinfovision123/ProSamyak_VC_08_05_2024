...
con3 =C.getConnection();
con4 =C.getConnection();
conDelete =C.getConnection();
...
//out.print("<br>63 UserID"+UserId);
//out.print("<br>64 EntityID"+EntityID);
  int p=0;
						
	//	SELECTS ALL ROWS IN DATABASE AND STORE IT IN DIFFERENT ARRAYS
if(command.equals("default"))
{
	String sql = "select MId, MName, ParentID, Child from MenuMaster where EID="+EntityID+" and Active=1 Order by ParentID,SrNo";
	pstmt1 = con1.prepareStatement(sql);
	rs1= pstmt1.executeQuery();
	out.println("<br><center><h2> Menu</h2></center>");
	...
	sql = "select count(*)as counter from MenuMaster where ParentID = -1 and EID="+EntityID+" and Active=1"; 
	pstmt1 = con1.prepareStatement(sql);
	rs1= pstmt1.executeQuery();
	int cnt = 0;
	while(rs1.next())
...
//out.print("<br>139 cnt="+cnt);
	for(j=0;j<cnt;j++)
		...
		<tr><td align="right" ><%=u++%>&nbsp;</td>
			<td align=left><a href="javascript:tb('InsertRow.jsp?command=Default&ParentId=<%=MenuID[j]%>&EntityID=<%=EntityID%>')" ><%=MenuName[j]%></a></td>
			<td >&nbsp;&nbsp;</td>
			<td >&nbsp;&nbsp;</td>
			<td >&nbsp;&nbsp;</td>
...
// IF CHILD PROPERTY TRUE THEN SEARCH ALL CHILDS(SECOND LEVEL)
		...
		String Query = "select MId, Child, MName from MenuMaster where ParentID = "+MenuID[j]+" and Active=1 order by SrNo";
...
%>
		<tr>	
			<td  align = "right"><%=(u-1)+"."+(v++)%>&nbsp;&nbsp;</td>
				<td align="center" >&nbsp;</td>
			<td ><a href="javascript:tb('InsertRow.jsp?command=Default&ParentId=<%=MenuID1[k]%>&EntityID=<%=EntityID%>')" > <%=rs2.getString("MName")%></a></td>
				<td >&nbsp;&nbsp;</td>
				<td >&nbsp;&nbsp;</td>
		</tr>
		...
		if(Child1[k] == true)
			{
				...
				Query = "select MId, Child, MName  from MenuMaster where ParentID = "+MenuID1[k]+" and Active=1 order by SrNo";
...
%>
					<tr>
						...
						<td align="center" >&nbsp;</td>
						<td align="center" >&nbsp;</td>
			<td ><a href="javascript:tb('InsertRow.jsp?command=Default&ParentId=<%=MenuID2[m]%>&EntityID=<%=EntityID%>')" ><%=temp%></a></td>
						<td >&nbsp;&nbsp;</td>
					</tr>
				...
				if(Child2[m]==true)
					{
						...
						Query = "select MId, Child, MName from MenuMaster where ParentID = "+MenuID2[m]+" and Active=1 order by SrNo";
...
%>						<tr>
						...
						<td >&nbsp;&nbsp;<td>
						<td align="center" >&nbsp;</td>
			<td ><a href="javascript:tb('InsertRow.jsp?command=Default&ParentId=<%=MenuID3[n]%>&EntityID=<%=EntityID%>')"> <%=rs4.getString("MName")%></a></td>
						</tr>
<%						n++;
						}	 
						pstmt4.close();
...
<%
  ...
  String sql1 = "select count(*) as counter from MenuMaster where Active=1 ";
int counter=0;
...
Boolean [] Childcnt = new Boolean[counter];
 sql = "select * from MenuMaster where Active=1 order by Sequence, SrNo  ";
	 //out.print("sql" +sql);
	pstmt1 = con1.prepareStatement(sql);
	rs1= pstmt1.executeQuery();
...
<%	
	C.returnConnection(conDelete);
	C.returnConnection(con1);
	C.returnConnection(con2);
	C.returnConnection(con3);
	C.returnConnection(con4);
	C.returnConnection(conIns);
