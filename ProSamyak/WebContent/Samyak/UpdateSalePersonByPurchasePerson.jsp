...
<!-- http://samyak2:8080/Nippon/Samyak/UpdateSalePersonByPurchasePerson.jsp?company_id=2   -->
...
for(int i=0;i<person_id.length;i++)
{
	...
	pstmt_g.close();
	errLine="83";
	insert_person="Update Receive set salesPerson_Id=? WHERE     (Company_Id = "+company_id+") AND (Receive_Sell = 1) AND (StockTransfer_Type = 0) AND (Active = 1) AND (R_Return = 0) AND (Opening_Stock = 0)   and salesPerson_Id="+person_id[i];
	pstmt_g = cong.prepareStatement(insert_person);
