...
function getNextBrokerPurchasePerson_header(Treceive_no,Tcompany_id,TisNext,Tname,TId,TButton)
{
...
//	alert("isNext_header="+isNext_header);
	...
	getXmlNextBrokerPurchasePersonIdName_header();
	
} //getNextReceiveNo

function getXmlNextBrokerPurchasePersonIdName_header()
{
	var req = newXMLHttpRequest();
	req.onreadystatechange = getReadyStateHandler(req, getBrokerPurchasePersonIdName_header);
 
	req.open("POST", "../BrokerPurchasePerson.do", true);
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

	req.send("action=getSubGroupLedger&company_id="+company_id_header+"&BrokerPurchasePerson_name="+receive_no_header+"&isNext="+isNext_header);
 
} //getXmlSubGroup()



function getBrokerPurchasePersonIdName_header(lotXML) {
	...
	if(lots.length==0)
	 {
	   alert("BrokerPurchasePerson Not Found");
