...
-------------------------------------------------------------------------------------------------------
1		01/09/09	Chanchal	CVD010909		To set max date till 2015 where ever hardcoded in js
												therewas no changes just reincoperated file with comment
------------------------------------------------------------------------------------------------------
*/
...
function fnCheckDate(astrDate,astrFieldName,sdate,edate)
{
	...
	//alert(astrFieldName);
	//alert(sdate);
//alert("15 SamyakYearEndDate"+edate);
//alert(today.getDate()+"/"+today.getMonth()+"/"+today.getYear());
...
//astrDate.select();
alert("The Date You Have Entered is "+astrDate);
mainform.datevalue.select();
mainform.datevalue.value=todayDate;
	return_flag=false;
}	
...
if(isNaN(name.value)) 
{ alert("Please Enter Number Properly");name.value="0"; name.focus();}
if(name.value.charAt(0) == ".") 
{ name.value="0"+name.value+""; }
var val=name.value;
 temp= new String(val);
...
function fnCheckDate_new(astrDate,astrFieldName,sdate,edate)
{
	alert("Inside SamyakYearEndDate"+astrDate);
//	alert(astrFieldName);
//	alert(sdate);
//	alert(edate);
var lstrDate = astrDate				//Local variable containing the argument
...
//astrDate.select();
	...
	if(return_flag)
	{
		addDueDays(document.mainform.datevalue,'Date',document.mainform.duedate,document.mainform.duedays);
	}
