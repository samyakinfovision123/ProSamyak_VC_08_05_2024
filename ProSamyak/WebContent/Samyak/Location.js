...
function getNextLocation_header(Treceive_no,Tcompany_id,TisNext,Tname,TId,TButton)
{
	...
	getXmlNextLocationIdName_header();
	
} //getNextReceiveNo

function getXmlNextLocationIdName_header()
{
	var req = newXMLHttpRequest();
	req.onreadystatechange = getReadyStateHandler(req, getLocationIdName_header);
 
	req.open("POST", "../Location.do", true);
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

	req.send("action=getSubGroupLedger&company_id="+company_id_header+"&Location_name="+receive_no_header+"&isNext="+isNext_header);
 
} //getXmlSubGroup()



function getLocationIdName_header(lotXML) {
