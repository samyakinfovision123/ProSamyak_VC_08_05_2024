...
String query="Select * from voucher where voucher_type=20  order by voucher_Id";
...
String newledger_id[]= new String[counter];
...
for( i=0;i<counter;i++)
{
 query="Select * from Financial_Transaction where For_Head=14 and voucher_id=? order by Tranasaction_Id";

pstmt_g = cong.prepareStatement(query);
pstmt_g.setString(1,voucher_id[i]);
rs_g = pstmt_g.executeQuery();
while(rs_g.next()) 	
{
 ft_id[i]= rs_g.getString("Tranasaction_Id");
...
newledger_id[i]=A.getNameCondition("Ledger", "Ledger_id","Where for_head=14 and ledger_type=2 and for_headid="+for_headid[i]+"");
}

out.print("<table border=1 bordercolor=green align=center cellspacing=2>");
out.print("<tr><th>no<th>Ft Id<th>Ledger_id<th>Party_id<th>New Led id");
int k=0;
...
out.print("<tr>");
out.print("<td>"+(++k)+"</td>");
out.print("<td>"+ft_id[i]+"</td>");
out.print("<td>"+ledger_id[i]+"</td>");
out.print("<td>"+for_headid[i]+"</td>");
out.print("<td>"+newledger_id[i]+"</td>");
out.print("</tr>");

}
for( i=0;i<counter;i++)
{
query="update Financial_Transaction set Ledger_id=? where Tranasaction_Id=?";
pstmt_p = conp.prepareStatement(query);
pstmt_p.setString (1, newledger_id[i]);
pstmt_p.setString(2,ft_id[i]);
