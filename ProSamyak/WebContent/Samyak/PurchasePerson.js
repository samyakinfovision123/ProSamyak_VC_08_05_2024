...
function getNextPurchasePerson_header(Treceive_no,Tcompany_id,TisNext,Tname,TId,TButton)
{
	//alert("Arreee");
	...
	getXmlNextPurchasePersonIdName_header();
	
} //getNextReceiveNo

function getXmlNextPurchasePersonIdName_header()
{
	var req = newXMLHttpRequest();
	req.onreadystatechange = getReadyStateHandler(req, getPurchasePersonIdName_header);
 
	req.open("POST", "../PurchasePerson.do", true);
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

	req.send("action=getSubGroupLedger&company_id="+company_id_header+"&PurchasePerson_name="+receive_no_header+"&isNext="+isNext_header);
 
} //getXmlSubGroup()



function getPurchasePersonIdName_header(lotXML) {
	...
	if(lots.length==0)
	 {
	   alert("PurchasePerson Not Found");
