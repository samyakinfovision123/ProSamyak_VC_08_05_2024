...
function getNextSalePurchaseGroupPerson_header(Treceive_no,Tcompany_id,Tgroupperson_type,TisNext,Tname,TId,TButton)
{
	...
	groupperson_type_header=Tgroupperson_type;
	//alert("groupperson_type_header="+groupperson_type_header);
	...
	getXmlNextSalePurchaseGroupPersonIdName_header();
	
} //getNextReceiveNo

function getXmlNextSalePurchaseGroupPersonIdName_header()
{
	var req = newXMLHttpRequest();
	req.onreadystatechange = getReadyStateHandler(req, getSalePurchaseGroupPersonIdName_header);
 
	req.open("POST", "../SalePurchaseGroupPerson.do", true);
	req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

	req.send("action=getSubGroupLedger&company_id="+company_id_header+"&groupsalepurchaseperson_type="+groupperson_type_header+"&PurchaseSaleGroup_name="+receive_no_header+"&isNext="+isNext_header);
 
} //getXmlSubGroup()



function getSalePurchaseGroupPersonIdName_header(lotXML) {
	...
	if(lots.length==0)
	 {
	   alert("GroupSalePurchasePerson Not Found");
