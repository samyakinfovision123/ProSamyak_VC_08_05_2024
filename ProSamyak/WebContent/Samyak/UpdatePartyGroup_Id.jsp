...
try	{
	 cong=C.getConnection();
		...
		{ 
		 out.println("<font color=red> FileName : 	UpdatePartyGroup_Id.jsp<br>Bug No Samyak31 : "+ Samyak31);
		}
...
/* 	Code for to update PartyGroup_Id for "Ledger" Table 
Start
*/
...
{
out.print("97");
String partygroup_id = A.getNameCondition(cong,"Master_PartyGroup", "PartyGroup_Id", "where Group_Type=0 and Active=1 and Company_Id="+company_id[tempi]+" and YearEnd_Id="+yearend_id[tempi]+"");


query="Update "+table_name+" set PartyGroup_Id=? where company_id=? and yearend_Id=? and For_Head=14 and Ledger_Type=1";

pstmt_g = cong.prepareStatement(query);
pstmt_g.setString(1,""+partygroup_id); 
pstmt_g.setString(2,""+company_id[tempi]); 
pstmt_g.setString(3,""+yearend_id[tempi]); 
out.print("<br>"+query);
tempx = pstmt_g.executeUpdate();

partygroup_id = A.getNameCondition(cong,"Master_PartyGroup", "PartyGroup_Id", "where Group_Type=1 and Active=1 and Company_Id="+company_id[tempi]+" and YearEnd_Id="+yearend_id[tempi]+"");


query="Update "+table_name+" set PartyGroup_Id=? where company_id=? and yearend_Id=? and For_Head=14 and Ledger_Type=2";
...
End
*/
out.println("PartyGroup_Id Updated successfully"+tempx);
pstmt_g.close();
