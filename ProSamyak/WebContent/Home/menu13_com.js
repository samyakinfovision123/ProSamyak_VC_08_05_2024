...
function KeepPos(){
	...
	if(TS!=StaticPos){FrstCntnr.OrgTop=StartTop+TS;FrstCntnr.style.top=FrstCntnr.mif.style.top=FrstCntnr.OrgTop+P_X;StaticPos=TS}}
...
function PosMenu(Ct,Tp,Lt){
	...
	while(Mb!=null){
		...
		if(Mb.CCn){
			...
			else{	SLt=RightToLeft?Li-CCw+ChilSamyakrlap*MWt+BRW:Li+(1-ChilSamyakrlap)*MWt+BRW;
...
function OpenMenuII(Ptr){
	...
	if(Ptr.Arr[3]&&!Ptr.CCn){
		...
		if(Hi){STp=BottomUp?Ti-CCh:Ti+MHt+2*BRW;SLt=RightToLeft?Li+MWt-CCw:Li}
		else{	SLt=RightToLeft?Li-CCw+ChilSamyakrlap*MWt+BRW:Li+(1-ChilSamyakrlap)*MWt;
...
function Unfld(){
	...
	if(MenuWrap){
		if(RightToLeft){if(SLt<LS)SLt=P.Lvl==1?LS:SLt+(CCW+(1-2*ChilSamyakrlap)*TWt);if(SLt+CCW>SWinW+LS)SLt=SWinW+LS-CCW}
		else{if(SLt+CCW>SWinW+LS)SLt=P.Lvl==1?SWinW+LS-CCW:SLt-(CCW+(1-2*ChilSamyakrlap)*TWt);if(SLt<LS)SLt=LS}
...
function CreateMenuStructure(MNm,No,Mcllr){
	...
	if(ExpYes)Lctn.document.body.appendChild(MIF);
