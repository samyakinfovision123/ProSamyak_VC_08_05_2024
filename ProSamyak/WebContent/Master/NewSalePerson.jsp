...
<script language=javascript src="../Samyak/SalePerson.js"></script>
...
<script language="javascript">
	...
	<%
		errLine="72";
			String SalePersonArray = "";
			String SalePersonId = "";
			String SalePersonQuery = "Select SalesPerson_Name,SalesPerson_Id from Master_SalesPerson where Active in(1,0) and Company_Id="+company_id+" and purchasesale=0 order by SalesPerson_Name";
		errLine="76";		
			pstmt_g = cong.prepareStatement(SalePersonQuery, ResultSet.TYPE_SCROLL_INSENSITIVE,                   ResultSet.CONCUR_READ_ONLY);
			...
			while(rs_g.next()) 	
			{
				if(rs_g.isLast())
				{
					SalePersonId += "\"" +rs_g.getString("SalesPerson_Id")+"\"";
					SalePersonArray += "\"" +rs_g.getString("SalesPerson_Name")+"\"";
			...
			pstmt_g.close();
		errLine="223";	
			out.print("var SalePersonArray=new Array("+SalePersonArray+");");
...
function editCancel(no)
{
	if(no==1)
	{
		...
		var str="../Master/SalesPerson.jsp?command=Default&message=Sale";
	...
	if(no==2)
	{
		...
		//var str="../Master/EditSalesPerson.jsp?command=edit&message=Sale";
var str="../Master/EditSalePurchaseBrokerPerson.jsp?command=Default&salepurchasebroker_id=0&salesperson_id="+document.new_mainform.cancel_receiveid.value;
	...
	if(no==3)
	{
		...
		var str="../Master/EditSalesPerson.jsp?command=edit&message=Sale&view=yes";
...
{
	last_button_pressed="2"; 
	getNextSalePerson_header(document.new_mainform.cancel_invno.value,'<%=company_id%>','Edit',document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
	return false;
} 
function check()
{
...
{	
	...
	getNextSalePerson_header(document.new_mainform.cancel_invno.value,'<%=company_id%>',s,document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
...
var SalePersonobj = new  actb(document.getElementById('cancel_invno'), SalePersonArray);
...
{ out.println("<br><font color=red> FileName : NewSalePerson.jsp Bug No Samyak499 : "+ Samyak499+"Error on"+errLine);
}
%>
</center>
</form>
