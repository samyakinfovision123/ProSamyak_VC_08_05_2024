...
if("Update".equals(command))
{
	...
	double nextOpeningR_Id[]=new double[m];
	double nextOpeningRT_Id[]=new double[m];
...
{ 		
	 ...
	 Location_Id[k]=Double.parseDouble(request.getParameter("Location_Id"+k)); 
	...
	//out.print("<br>Receive_Id"+k+Receive_Id[k]);
		...
		//out.print("<br>lotid"+k+lotid[k]);

		exchangrate[k]=Double.parseDouble(request.getParameter("ExchangRate"+k));
		//out.print("<br>Exchange Rate"+k+exchangrate[k]);
		quantity[k]=Double.parseDouble(request.getParameter("quantity"+k));
		//out.print("<br>Quantity"+k+quantity[k]);

		nextOpeningR_Id[k] = Double.parseDouble(request.getParameter("nextOpeningR_Id"+k));
		//out.print("<br>nextOpeningR_Id"+k+nextOpeningR_Id[k]);

		nextOpeningRT_Id[k] = Double.parseDouble(request.getParameter("nextOpeningRT_Id"+k));
...
//Start : New opening year stock value updation in Receive table
...
//		out.print("<br>7");
		...
		pstmt_p.setString(12,""+nextOpeningR_Id[k]);
		
		  a224 = pstmt_p.executeUpdate();
		pstmt_p.close();

	//End : New opening year stock value updation in Receive table
...
//out.print("4as**");
	...
	//Start : New opening year stock value updation in Receive Transaction table
...
//out.print("as**");
		...
		pstmt_p.setBoolean (9,true);	
	 	pstmt_p.setDouble(10,Location_Id[k]);	
		pstmt_p.setDouble(11,nextOpeningRT_Id[k]);	
...
//out.print("3as**");
	...
	//End : New opening year stock value updation in Receive Transaction table
	response.sendRedirect("EditClosingStock.jsp?command=edit&message=<center><font class=msgred>Data Successfully Updated</font></center>");
