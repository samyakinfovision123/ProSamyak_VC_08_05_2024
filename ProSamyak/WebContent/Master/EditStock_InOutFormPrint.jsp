...
int scounter=0;
String query1="Select Receive_Lots,receive_currencyId from receive where receive_id="+sreceive_Id;
...
//	out.print("<br> 163 Except Loss");
	...
	query="select ReceiveTransaction_Id,Lot_Id, Quantity, Local_Price,Dollar_Price,Location_Id from Receive_Transaction where Receive_Id="+dreceive_Id+" order by Lot_SrNo";
	errLine="253";
 ...
 <tr>
 <th colspan=10 align=center> Stock In / Out</th>
 </tr>
 <tr>
	<td colspan=10>
	<table  width='100%'>
	<tr>
	<td colspan=2>&nbsp;&nbsp;Inv No:<%=voucher_no%>
	</td>
	<td colspan=2>&nbsp;&nbsp;Ref. No:
	<%=ref_no%></td>
	<td colspan=2>&nbsp;&nbsp;Transfer Date
	<%=today_date%></td>
	...
	<%=str.mathformat(""+base_exchangerate,2)%>
	</td>
	</tr>
	</table>
	</td></tr>
...
<tr>
	<td><%=i+1%></td>
	<td ><%=A.getNameCondition(conp,"Lot","Lot_No","where Lot_Id="+dlotid[i])%></td>
     
	
		
	
	
	<td align=right><%=dqty[i]%>
	</td>
	...
	<td align=right><%=ddollaramount[i]%></td>

	<td align=right><%=damount[i]%></td>

	<td><%=A.getNameCondition(conp,"Lot","Lot_No","where Lot_Id="+slotid[i])%>
...
<tr>
	...
	<td align=right><%=dtotalqty%></td>
	<td colspan=2>&nbsp;</td>
	<td align=right><%=dtotaldollaramount%></td>	
	<td align=right><%=dtotalamount%></td>
	<td>&nbsp;</td>
</tr>
	...
	<tr>
	 ...
	 for(int i=0;i<scounter;i++)
		...
		<tr>
			...
			<td align=right ><%=str.mathformat(""+sqty[i],3)%>
			...
			<td align=right><%=str.mathformat(""+srate[i],3)%>
			
			</td>
			<td align=right><%=str.mathformat(""+slocalrate[i],3)%></td>
		...
		%>
			<tr>
				<td colspan=2 align=right>Total :&nbsp;&nbsp;</td>
				<td align=right><%=str.mathformat(""+stotalqty,3)%></td>
				<td colspan=2>&nbsp;</td>
				<td align=right><%=str.mathformat(""+stotalamt,d)%></td>
...
<% if(dcounter>0)
		...
		{%>
			<tr>
				...
				<input type=hidden align=right name="dqty<%=i%>" value="<%=dqty[i]%>">
				</td>
	...
	<%	}%>
			<tr>
				...
				<td colspan=2>&nbsp;</td>
				<td align=right><%=str.mathformat(""+dtotalamt,d)%></td>
