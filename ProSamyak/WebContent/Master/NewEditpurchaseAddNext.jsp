...
try{
	...
	if("lot".equals(addType))
	{
		...
		System.out.print("<br> companyparty_name"+companyparty_name);
		String companyparty_id = request.getParameter("companyparty_id");
		System.out.print("<br> companyparty_id"+companyparty_id);
		...
		int receivetransaction_id[]=new int[tempcounter]; 
		...
		double totalSplit_Quantity=Double.parseDouble(request.getParameter("totalSplit_Quantity"));
ErrLine="242";
		int splitFlag=Integer.parseInt(request.getParameter("splitFlag"));
		int SplitCounter=Integer.parseInt(request.getParameter("SplitCounter"));
		int PFlag=Integer.parseInt(request.getParameter("PFlag"));

		double Split_Quantity[]= new double[tempcounter];
		int Split_RTID[]= new int[tempcounter];
		String split_date ="" ;
ErrLine="250";
		if(splitFlag == 1)
		{	for( i=0;i<tempcounter;i++)
			{
			Split_Quantity[i]=Double.parseDouble(request.getParameter("Split_Quantity"+i));
			Split_RTID[i]=Integer.parseInt(request.getParameter("Split_RTID"+i));

			}
			split_date=request.getParameter("split_date");
		}
ErrLine="260";
String Narration=request.getParameter("narration");
//System.out.println("Narration"+Narration);
String NarrationTemp=request.getParameter("narrationtemp");
//NarrationTemp=Narration;
	if("ADD".equals(Command))
	{
		...
		function recalculateQty(rowNum)
		{
			...
			var tempCounter=parseFloat(document.mainform.tempcounter.value);
			
			var split_Flag=parseFloat(document.mainform.splitFlag.value);
			
			if(rowNum < tempCounter)
			{ 			
				if(split_Flag == 1 )
				{
...
// end $ rate find	
		...
		if(command == "NEXT")
		{
			...
			//{ alert(" Sorry ,Your Invoice Total Quantity is less than.. It's  Split Quantity");
			//return false;
			//}
		}
		return true;
...
{
	var split_Flag=document.mainform.splitFlag.value;
	...
	return true;
		...
		<tr>
			...
			<td>
				<%=AC.getMasterArrayCondition(cong,"PurchaseSaleGroup","purchasesalegroup_id",purchasesalegroup_id,"where Active=1 and PurchaseSaleGroup_Type=1",company_id)%>
	...
	<TABLE borderColor=skyblue WIDTH="100%" align=center border=1  cellspacing=0 cellpadding=2>
		...
		<td  width="15%">
			<input type=text name='datevalue' size=8 maxlength=10 value="<%=datevalue%>" OnKeyUp="checkMe(this);" onBlur="return checkSplitDate()"></td>	
	...
	<tr>
		...
		{	//System.out.println("1034  Under if " +i);	
			if(DelLots[i] == null)
			{
			  if(splitFlag == 1)
			  { 
				if(Split_RTID[i]==receivetransaction_id[i])
				{%><td><%=t%></td><%}
				else
		...
		<td><input type=text name=lotno<%=i%> id=newlotno<%=i%> autocomplete=off value="<%=lot_no[i]%>" size=7 style="text-align:right" onblur="getDescSize('<%=company_id%>', document.mainform.lotno<%=i%>.value, document.mainform.datevalue.value, 'lotid<%=i%>','description<%=i%>', 'dsize<%=i%>', 'effrate<%=i%>', 'description<%=i%>', 'purchaseEdit' ); " autocomplete=off onfocus="this.select()"
		<%if(splitFlag == 1)
		  { if(i < tempcounter){
			if(Split_RTID[i]==receivetransaction_id[i]){out.print(" readonly style=background:#CCCCFF");}}}
		%>></td>
		<%}else{%>
			<td><input type=text name=lotno<%=i%> id=newlotno<%=i%> autocomplete=off value="<%=lot_no[i]%>" size=7 style="text-align:right" onblur="getDescSize('<%=company_id%>', document.mainform.lotno<%=i%>.value, document.mainform.datevalue.value, 'lotid<%=i%>','description<%=i%>', 'dsize<%=i%>', 'effrate<%=i%>', 'description<%=i%>', 'purchase' ); " autocomplete=off onfocus="this.select()" 
			<%if(splitFlag == 1)
		...
		<%if(i < tempcounter){%>
		<td><input type=text  name=dsize<%=i%> size=4 value="<%=size[i]%>"  style="text-align:left" id=dsize<%=i%> autocomplete=off onfocus="this.select()" onblur="getLots('<%=company_id%>', document.mainform.description<%=i%>.value, document.mainform.dsize<%=i%>.value, document.mainform.datevalue.value, 'lotid<%=i%>','lotno<%=i%>', 'effrate<%=i%>', 'description<%=i%>', 'purchaseEdit' ); "></td>
		<%}else{%>
			<td><input type=text  name=dsize<%=i%> size=4 value="<%=size[i]%>"  style="text-align:left" id=dsize<%=i%> autocomplete=off onfocus="this.select()" onblur="getLots('<%=company_id%>', document.mainform.description<%=i%>.value, document.mainform.dsize<%=i%>.value, document.mainform.datevalue.value, 'lotid<%=i%>','lotno<%=i%>', 'effrate<%=i%>', 'description<%=i%>', 'purchase' ); "></td>
			<%}%>
		...
		<%}else{%>
		<td><input type=text name=originalQuantity<%=i%> value="<%=originalQuantity[i]%>"  size=5 style="text-align:right"onBlur=" recalculateQty('<%=i%>'); setOtherRates('<%=i%>');" onfocus="this.select();  setRatesAfterLotFetch(<%=i%>);"></td>
		<%}%>
		...
		<!-- Add -->
		<td><input type=hidden name=rate<%=i%> size=7 value="<%=str.mathformat1(rate[i], 3)%>"  style="text-align:right" id=rate<%=i%>>
		
		<input type=text onfocus="this.select()" size=5 name=effrate<%=i%> value="<%=str.mathformat1(effrate[i], 3)%>"  style="text-align:right" onBlur="calculateEffRate('<%=i%>')" id=effrate<%=i%> <%if(currency_id != 0)
		...
		<!-- end -->
		<td><input type=text name=amount<%=i%> value="<%=str.format2(""+amount[i],d)%>" size=7 style="text-align:right;background:#CCCCFF""></td>

		<td><input type=text name=Localamount<%=i%> value="<%=str.format2(""+Localamount[i],d)%>" size=7 style="text-align:right;background:#CCCCFF"></td>
...
<%}
		...
		{%>
			...
			<%if(splitFlag == 1)
			{%>				
				<input type=hidden name=Split_Quantity<%=i%> value="<%=Split_Quantity[i]%>">
				<input type=hidden name=Split_RTID<%=i%> value="<%=Split_RTID[i]%>">
			<%}

		}
		 if(splitFlag == 1)
		{%>
			<input type=hidden name=split_date value=<%=split_date%> >
		<%}
	...
	for(i=lotRows;i<lotRows+newLotRows;i++)
	{
		...
		<td><input type=text name=lotno<%=i%> id=lotno<%=i%> autocomplete=off value="" size=7 style="text-align:right" onblur="getDescSize('<%=company_id%>', document.mainform.lotno<%=i%>.value, document.mainform.datevalue.value, 'lotid<%=i%>','description<%=i%>', 'dsize<%=i%>', 'effrate<%=i%>', 'description<%=i%>', 'purchase' );" onfocus="this.select()" autocomplete=off></td>
		...
		<td><input type=text  name=dsize<%=i%> size=4 value=""  style="text-align:left" id=dsize<%=i%> autocomplete=off onfocus="this.select()" onblur="getLots('<%=company_id%>', document.mainform.description<%=i%>.value, document.mainform.dsize<%=i%>.value, document.mainform.datevalue.value, 'lotid<%=i%>','lotno<%=i%>', 'effrate<%=i%>', 'description<%=i%>', 'purchase' ); "></td>

		<td><input type=text name=originalQuantity<%=i%> value="0"  size=5 style="text-align:right" onBlur=" recalculateQty('<%=i%>'); setOtherRates('<%=i%>');" onfocus="this.select();  setRatesAfterLotFetch(<%=i%>);"></td>
		...
		<!-- Add -->
		<td>
			<input type=hidden name=rate<%=i%> size=5 value="1"  style="text-align:right" id=rate<%=i%>>
		...
		<td><input type=text name=lotDiscount<%=i%> value="0" size=7 style="text-align:left" onBlur="checkDiscount(this);<%
		if(currency_id != 0)
		{
			out.print("if(document.mainform.currency[1].checked )");
		}
	...
	for( j=0;j<oldLedgerRows;j++)
		{
			...
			//out.print("<br> 790 ledger"+ledger[j]);
...
//-----------------------------------------------------------------//
	if("NEXT".equals(Command))
	{
		...
		//System.out.println("1579 NarrationTemp"+NarrationTemp);
		%>
		<html>
		<head>
		</head>			
		<body background="../Buttons/BGCOLOR.JPG" >
		<form name=mainform action="NewEditpurchaseUpdate.jsp" method="post">
			
		<TABLE borderColor=skyblue WIDTH="100%" align=center border=1  cellspacing=0 cellpadding=2>
		<tr>
		<th colspan=8 align=center>Edit Purchase - Invoice </th>
		</tr>	
		</table>
		<TABLE borderColor=skyblue WIDTH="100%" align=center border=1  cellspacing=0 cellpadding=2>
		<tr>
	...
	<tr>
		...
		<td>Desc </td>
		<td>Size</td>
		<td>Original <br> Qty</td>
		<td>Return  <br>Qty</td>
		<td>Ghat <br> Qty</td>
		<td>Selection <br>Qty</td>
		<td>Split <br>Qty</td>
		...
		<td>Amount $ </td>
		<td>Amount (Rs)</td>
		<td>Lot <br> Discount %</td>
		<td>Remarks</td>
	</tr>
		<%ErrLine="1276";
		for(i=0;i<oldCounter;i++)
		{  t=i; t++;
			double tempQuantity=Double.parseDouble(quantity[i]);
			if("".equals(lot_id[i]))
			{
				blankLots++;
				continue;
			...
			{
				if( DelLots[i] == null )
				{%>	<input type=hidden name=checkBox<%=i%>>
					<tr>
				<td align=center>
				<table bordercolor=skyblue border=0>
				<tr>
				<%if(splitFlag == 1)
				  { if(Split_Quantity[i] == tempQuantity)
					{%><td><%=t%></td><%}
					else{%><td><%=t%></td><!-- <td><input type=checkBox name=splitCheckBox<%//=i%> > </td>--><%}
				  }
				 else
				 {%>
				  <!-- <td><input type=checkBox name=splitCheckBox<%//=i%> ><td></td>	 -->
				<%}%>
				</tr></table></td>
			<%}
			else{%>
			...
			<%}else{%><tr>
				...
				<table>
				<tr><td><%=t%></td>
				<!-- <td><input type=checkBox name=splitCheckBox<%//=i%> --> </td>	
					</tr></table></td><%}%>
			...
			<%if(i<tempcounter )
				{ if(splitFlag == 1)
				  {%>
					<td align=right><%=Split_Quantity[i]%>
				  <%}else{%><td align=right><%=0%>
				<%}
		...
		{%>
			...
			{%>				
				<input type=hidden name=Split_Quantity<%=i%> value="<%=Split_Quantity[i]%>">
			<%//System.out.println(" Edit Pur AddNext Split_Quantity"+Split_Quantity[i]);%>
				<input type=hidden name=Split_RTID<%=i%> value="<%=Split_RTID[i]%>">
			<%}
		}%>	
			<input type=hidden name=splitFlag value="<%=splitFlag%>">
			<tr>
				...
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td align=right> <%=str.format2(""+InvTotalAmount,d)%><input type=hidden name="InvTotalAmount" value="<%=str.format2(""+InvTotalAmount,d)%>" ></td>
				<td align=right> <%=str.format2(""+InvTotalLocalAmount,d)%><input type=hidden name="InvTotalLocalAmount" value="<%=str.format2(""+InvTotalLocalAmount,d)%>" ></td>
				<td>&nbsp;</td>
		</tr>
	...
	<input type=hidden name=companyparty_id value="<%=companyparty_id%>">
		...
		<tr>
			...
			<td align=right><%=str.format2(""+TotalAmount,d)%>
				<input type=hidden name="totalamount" value=<%=str.format2(""+TotalAmount,d)%>></td>
			<td align=right><%=str.format2(""+TotalLocalAmount,d)%>
				<input type=hidden name="totalLocalamount" value=<%=str.format2(""+TotalLocalAmount,d)%>></td>
		</tr>
		<tr>
			<td>Narration : </td>
	...
	<TABLE borderColor=skyblue WIDTH="100%" align=center border=0  cellspacing=0 cellpadding=2>
		...
		<td>&nbsp;</td>
			<td><input type=checkBox name=splitCheckBox1> <b> Split Invoice<b></td><td><input type=submit name=command  value=SAVE class='button1' onmouseover="this.className='button1_over';" onmouseout="this.className='button1';" >
			</td>
