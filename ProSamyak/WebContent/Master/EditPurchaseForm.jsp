...
//System.out.println("usDlr"+usDlr);
...
double PT_LocalAmount=0.0;
	...
	query = "select Challan_no from receive where receive_id="+receive_id;
	pstmt_p = cong.prepareStatement(query);
	rs_p = pstmt_p.executeQuery();	
	while(rs_p.next())
	{
		Challan_no = rs_p.getInt("Challan_no");
	...
	if (currency_id == 0)
	{
		...
		{%>
			...
			<body>
				<!-- <center> <br><br><h3><font color=red> kSorry,For The Voucher Payment is Done.. Then.. You Cant Edit This Voucher.. </font></h3></center> -->
			</body></html>
		...
		<head><title>Samyak Software</title></head>
		<body>
		<!-- <center><br><br>	<h3><font color=red> jSorry,For This Voucher Payment is Done.. Then.. You Cant Edit This Voucher..</font> </h3>
		</center> --></body></html>
...
//out.print("<br> 334 locaation_id "+location_id[n]);
...
function calcTotal(name)
{
	...
	for(i=0;i<counter;i++)
	{
		...
		var amt=0;
		//alert("deletedName="+deletedName);
		//alert(document.mainform.elements[deletedName].checked);
		...
		//var amount11=document.mainform.elements[amountName].value;
...
%>
<%if(Challan_no!=0 || PFlag == 1){%>
	
<input type=text onfocus="this.select()" name=companyparty_name value='<%=A.getNameCondition(conp,"Master_CompanyParty", "CompanyParty_Name", " Where CompanyParty_Id="+companyparty_id)%>'  size=15 id=companyparty_name autocomplete=off readonly style="background:#CCCCFF">
<%}else{%>
...
onblur='return  fnCheckDate(this.value,"Date",<%out.print("\""+startDate+"\"");%>,<%out.print("\""+endDate+"\"");%>);'
 onfocus='stdate();' class="ipinvoice" accesskey="i" tabindex=1 onchange='stdate();' style="background:#CCCCFF" readonly>
</td>
<%
	}
	else
...
<input type=hidden name="PaymentFlag" value="<%=PaymentFlag%>">
...
<td><%=AC.getMasterArrayCondition(conp,"SalesPerson","salesperson_id",salesperson_id,"where PurchaseSale=1 and Active=1 and company_id="+company_id+" order by SalesPerson_Name") %>
</td>
...
<td>Rate / Unit</td>
<td>Amount in <input type=text name=currency_text size=2 value="<%=local_currencysymbol_Show%>" readonly ></td>
<td>Remarks</td>
</tr>
<% 
for (int i=0;i<counter;i++)
...
if((n+ledgers)==1)//n:no of lots ledgers: no of ledgers
	{
		%>
<tr><td><%=i+1%><input type=hidden name="deleted<%=i%>" value="no"></td>
<%}
    else
  ...
  <%}%>
	<!-- onclick=' return calcTotal(document.mainform.ctax_amt)'> -->
...
%>
<td>
<input type=text name=newlotno<%=i%> id=newlotno<%=i%> autocomplete=off value="<%=lot_no[i]%>" size=10 onBlur='return nullvalidation(this)'  onfocus='handleFocus("newlotno<%=i%>")' style="background:#CCCCFF" readonly></td>
<td><input type=text name=quantity<%=i%> value="<%=quantity[i]%>" size=5 OnBlur='validate(this,3)' style="text-align:right" style="background:#CCCCFF" readonly></td>
<td><input type=text name=rate<%=i%> value="<%=rate[i]%>" size=5  style="text-align:right" style="background:#CCCCFF" readonly></td>
<td><input type=text name=amount<%=i%> value="<%=str.mathformat(""+amount[i],2)%>" size=8 style="text-align:right" OnBlur='return calcTotalRow(this, document.mainform.deleted<%=i%>, document.mainform.newlotno<%=i%>, document.mainform.rate<%=i%>, document.mainform.amount<%=i%>, document.mainform.quantity<%=i%>, document.mainform.hiddenamount<%=i%>)' style="background:#CCCCFF" readonly>
...
<%
	...
	{
%>
<td><input type=text name=newlotno<%=i%> id=newlotno<%=i%> autocomplete=off value="<%=lot_no[i]%>" size=10 onBlur='return nullvalidation(this)'  onfocus='handleFocus("newlotno<%=i%>")'></td>
<td><input type=text name=quantity<%=i%> value="<%=quantity[i]%>" size=5 OnBlur='validate(this,3)' style="text-align:right"></td>
<td><input type=text name=rate<%=i%> value="<%=rate[i]%>" size=5  style="text-align:right"></td>
<td><input type=text name=amount<%=i%> value="<%=str.mathformat(""+amount[i],2)%>" size=8 style="text-align:right" OnBlur='return calcTotalRow(this, document.mainform.deleted<%=i%>, document.mainform.newlotno<%=i%>, document.mainform.rate<%=i%>, document.mainform.amount<%=i%>, document.mainform.quantity<%=i%>, document.mainform.hiddenamount<%=i%>)'>
<input type=hidden name=hiddenamount<%=i%> value="<%=str.mathformat(""+amount[i],2)%>">
</td>
<%}%>
...
<tr>
	...
	<input type="checkbox" name="findeleted<%=i%>" value="yes"> 
	</td>

	<td colspan=3 align=right>  <%=A.getArray(conp,"Ledger","ledger"+i,Ledger_Id[i],company_id +" and yearend_id="+yearend_id,"Expense") %> &nbsp;&nbsp;&nbsp;&nbsp;
	</td>
...
<%if(Challan_no!=0)
	{
%>
<td colspan=1><input type=text name=subtotal size=8 readonly value="<%=str.mathformat(""+subtotal,2)%>" style="text-align:right" style="background:#CCCCFF" readonly></td>
<%}
else
	{%>
	<td colspan=1><input type=text name=subtotal size=8 readonly value="<%=str.mathformat(""+subtotal,2)%>" style="text-align:right" style="background:#CCCCFF"></td>
<%}%>
...
<tr><input type=hidden name=PFlag value="<%=PFlag%>">
...
<td colspan=1><input type=text readonly name=total size=8 value="<%=str.mathformat(""+org_receivetotal,2)%>" style="text-align:right"></td><!-- OnBlur='return finalTotal()' -->
</tr>
...
<%}if(Challan_no!=0)
	{
%>
<input type=submit name=command value=ADD class='button1' disabled>
<%}
else
	{
	%>
...
function finalTotal()
{
	...
	//var finaltotal=temp_total+tax_amt;
	//alert("subtotal value="+document.mainform.subtotal.value);
	var finaltotal= parseFloat(document.mainform.subtotal.value)+
	parseFloat(document.mainform.ctax_amt.value);
	finaltotal=finaltotal.toFixed(d);
...
{
//var d=2;
//alert(document.mainform.currency[0].checked);
var fieldname=name.name;
...
if(fieldname!=null)
	{
if(document.mainform.lotno<%=i%>.value!="#")
{	
...
if(fieldname=="amount<%=i%>")
	{
if(document.mainform.newlotno<%=i%>.value!="#")
{	
...
<input type=hidden name="PaymentFlag" value="<%=PaymentFlag%>"></td>
</tr>
...
<tr>
 <td colspan=4>Sub Total in <%=currency_text%></td>
<td colspan=1><input type=text name=subtotal size=8 readonly value="<%=subtotal%>" style="text-align:right" style="background:#CCCCFF"></td>
<td colspan=1></td>
</tr>
...
<input type=hidden name=TotalPT_LocalAmount value="<%=TotalPT_LocalAmount%>">
 ...
 <td colspan=4>Total in <%=currency_text%></td>
...
out.println("<font color=red> FileName : EditPurchaseForm.jsp command=sedit<br>Bug No Samyak1031 : "+ Samyak1031+"ErrLine="+errLine);}
%>
...
//out.print(" 2895 Next masterlocation"+masterlocation);
//int addlots=Integer.parseInt(request.getParameter("addlots"));
int addlots=0;
String receive_id=request.getParameter("receive_id");
String lots = request.getParameter("no_lots");
...
newlocation_id[i]=""+request.getParameter("newlocation_id"+i);
//out.print("<br> 2996 newlocation_id[i]"+newlocation_id[i]);
...
<body bgColor=#ffffee   background="../Buttons/BGCOLOR.JPG">
<FORM name=mainform
action="EditPurchaseUpdate.jsp?command=UPDATE" method=post >
...
<td colspan=2>Ref.No:<%=ref_no%><input type=hidden name="ref_no" value="<%=ref_no%>"></td>
<td>From:</td><td><%=party_name%></td>
<!--<td>By:<%//=company_name%></td>-->
<td colspan=1>&nbsp;</td>
</tr>
	<tr>
...
if (!document.layers) {document.write("<input type=button class='button1' onclick='popUpCalendar(this, mainform.finalduedate, \"dd/mm/yyyy\")' value='Due Date' style='font-size:11px ; width:100'>")}
	...
	<input type=text name='finalduedate' size=8 maxlength=10 value="<%=finalduedate%>" onblur='return  fnCheckMultiDate1(this,"Due Date")'>
</td>
<td colspan=1>&nbsp;</td>
	</tr>
...
<tr>
  <td colspan=2>Location:<%=A.getName(conp,"Location",masterlocation)%></td>
	<input type=hidden name=masterlocation value=<%=masterlocation%>>
...
<td colspan=1>Purchase Group : <%=A.getNameCondition(conp,"Master_PurchaseSaleGroup","PurchaseSaleGroup_Name","where PurchaseSaleGroup_Id="+purchasesalegroup_id)%>
...
<input type=hidden name=category_id value='<%=category_id%>'>
 ...
 <%}
	...
	{%>
<td>Category : <%=A.getNameCondition(conp,"Master_LotCategory","LotCategory_Name","where LotCategory_Id="+category_id)%>
<input type=hidden name=category_id value='<%=category_id%>'>
</td>
	<%}%>
...
<td colspan=1>Exchange Rate <input type=hidden name=exchange_rate value="<%=str.format(exchange_rate)%>" onBlur="validate(this,2)"  size=4 style="text-align:right"><%=str.format(exchange_rate)%>
</td>
<td colspan=1>&nbsp;</td>
</tr>
...
<td align=right>Amount in <input type=text name=a value="<%=local_currencysymbol%>" size='1'></td>
<td>Remarks</td>
</tr>
...
<tr>
<td colspan=1>&nbsp;</td>
<td colspan=2> Sub Total in <%=local_currencysymbol%></td>
<td colspan=1 align="right"><%=str.format(""+tot_newqty,3)%></td>
<td colspan=1> &nbsp;</td>
<td colspan=1 align="right">
	<input type=hidden name=subtotal size=8 readonly value="<%=subtotal%>" style="text-align:right"><%=str.format(""+subtotal,d)%></td>
<td colspan=1>&nbsp;</td>
</tr>

<tr>
<td colspan=1>&nbsp;</td>
<td colspan=3>Tax (%)</td><td align=right>
<input type=hidden name=ctax size=8 OnBlur='return calcTotal(this)' value="<%=ctax%>" style="text-align:right"> <%=ctax%></td>
...
<td colspan=1 align="right"><input type=hidden readonly name=total size=8 value="<%=total%>" style="text-align:right"> <%=str.format(""+total,d)%> </td>
	<td colspan=1>&nbsp;</td>
...
<tr>
	<td>&nbsp;</td>
	<td colspan=6>Due Days<input type=hidden name=duedays size=1 value="<%=duedays%>" onBlur="validate(this)" style="text-align:right"> <input type=hidden name=duedaysold size=1 value="<%=duedaysold%>" onBlur="validate(this)" style="text-align:right"><%=duedays%></td></tr>
