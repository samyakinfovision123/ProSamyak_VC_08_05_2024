...
// end of today_date in dd/mm/yyyy format
	...
	int directSave = 0;
	errLine = "115";
	 String splitCheckBox=request.getParameter("splitCheckBox1");
 //System.out.println("319 splitCheckBox"+splitCheckBox);
	...
	//out.print("<br> tempcounter"+tempcounter);
	...
	if("SAVE".equals(command)  )
	{
		 ...
		 boolean dataCommitted = false; //just used forredirecting decision purpose
...
errLine = "137";
	...
	int newCompanyParty_Id=Integer.parseInt(A.getNameCondition(cong,"Master_CompanyParty","CompanyParty_Id","where Active=1 and companyParty_name like '"+companyparty_name+"'"));
...
errLine = "231";
	...
	//HashMap h=new HashMap();
		...
		for(int i=0;i<tempcounter;i++)
		{
			...
			//h.put(receivetransaction_id[i],i);
			//System.out.println(" 237 receivetransaction_id"+receivetransaction_id[i]);
...
errLine = "236";
		...
		//end delete lot..
/*
int splitcounter=lotRows - splitEqualCounter -DelCounter;
int splitreceiveTransaction_id[]=new int[splitcounter];

System.out.println(" 264 splitcounter"+splitcounter);
String SpliCheck[]=new String[splitcounter];
int x=0;
		for(int k=0;k<lotRows; k++)
		{
			if(lotRows > 1 && k<tempcounter)
			{
				if(Split_Quantity[k]==qty[k] || DelLots[k].equals("Delete"))
				{
				}else{
			SpliCheck[x]=request.getParameter("splitCheckBox"+x);
			splitreceiveTransaction_id[x]=receivetransaction_id[k];
			System.out.println("splitreceiveTransaction_id[x]"+splitreceiveTransaction_id[x]+"SpliCheck"+SpliCheck[x]);
				x++;
...
int splitCheckOnCounter=0;
	for(int k=0;k<splitcounter;k++)
	{
		if(SpliCheck[k] == null)
		{}
		else{
...
x=0;
System.out.println("splitCheckOnCounter"+splitCheckOnCounter);
int SelectForSplitLotRT_Id[]=new int[splitCheckOnCounter];
	...
	for(int k=0;k<splitcounter;k++)
	{
		...
		{}
				...
				if(h.containsKey(splitreceiveTransaction_id[k]))
				{
				SelectForSplitLotRT_Id[x]=splitreceiveTransaction_id[k];

		System.out.println(" 314 SelectForSplitLotRT_Id"+SelectForSplitLotRT_Id[x]);
				}
			}
	}
 */
...
errLine = "248";
		...
		//System.out.println("339 narration"+Narration);
		//System.out.println("339 TempNarration1"+TempNarration1);
errLine = "247";
...
int ghatReceive_id=0;
		...
		while(rs_g.next())
		{
			ghatReceive_id = rs_g.getInt("receive_id");
		}
		pstmt_g.close();
	errLine = "302";
		...
		for(int k=0;k<tempcounter;k++)
		{
			...
			while(rs_g.next())
			{
				ghatReceiveTransaction_id[k] = rs_g.getInt("receivetransaction_id");
				//out.print("<br> 525 ghatReceiveTransaction_id"+ghatReceiveTransaction_id);
			}
			pstmt_g.close();
...
//System.out.println(" after the seAutoCommit false..");
		...
		pstmt_p.close();
		//System.out.println("<br> 310 TempNarration1"+TempNarration1+"Update data on receive.."+a224);
		errLine = "312";
		...
		if(tempcounter != 0)
		{
			...
			{
			query="update Receive_Transaction set Lot_Id=?, Original_Quantity=?, Quantity=?, Available_Quantity=? , Receive_Price=?, Local_Price=?, LocalEffective_Price=?, Dollar_Price=?, DollarEffective_Price=?, Local_Amount=?, Dollar_Amount=?, Remarks=?, Return_Quantity=?, Lot_Discount=? , Modified_On=?, Modified_By=?, Modified_MachineName=?,Active=? where receivetransaction_id="+receivetransaction_id[i];
			pstmt_p = conp.prepareStatement(query);
			...
			if(splitFlag == 1)
			{
				qty[i]= qty[i] - Split_Quantity[i];
			}
			//System.out.println("429 qty[i]"+qty[i]+"receivetransaction_id[i]"+receivetransaction_id[i]);
			pstmt_p.setDouble (4,qty[i]);	
			if("Local".equals(currency))
			...
			pstmt_p.setDouble (10,Localamount[i]);		
			pstmt_p.setDouble (11,amount[i]);	
			pstmt_p.setString (12, remarks[i]);		
			pstmt_p.setDouble (13,returnQty[i]);
			...
			pstmt_p.close();
			//System.out.println("<br> 355 Update data on receive_transaction.."+a245);
		}
...
int ghatCgtRTId[]=new int[newLotrows];
	...
	errLine = "536";
			...
			//System.out.println("<br> 431 Insert data On RT for new Lots.."+i+"uPADET "+a245);
		}//END..FOR
	}//END IF
...
errLine = "598";
		pstmt_p.close();
		//System.out.println("<br> 510 Update ghatdata on receive.."+a807);
...
//UPDATE OF GHATDATA FOR EACH LOT IN THE RECEIVE_TRANSACTION
	...
	errLine = "648";
			...
			pstmt_p.close();
			//System.out.println("<br> 651 Update ghatdata on receive_transaction for old lot.."+i+"upadte"+a424);
		} 
	}
...
//**INSERT GHATDATA FOR EACH NEW LOT IN RECEIVE_TRANSACTION
	if(newLotrows != 0)
	{
		...
		errLine = "645";
				Receivetransaction_id1++;
				j++;
			//	System.out.println("<br> 718 insert ghat  data On RT for new Lots.."+i+"uPADTE ->"+a873);
...
// UPDATE DATA INTO THE VOUCHER TABLE..
		...
		pstmt_p.setString (6,TempNarration1);
...
//UPDATE GHATDATA INTO THE VOUCHER TABLE
	...
	errLine = "774";
			...
			pstmt_p.close();
		//	System.out.println("<br> 775 update..Voucher .."+ra691);
...
// UPDATE FINANCIAL_TRANSACTION FOR EACH OLD LEDGER..
	...
	errLine = "813";
			...
			//	System.out.println("811 update FT.. for old ledger.."+a525);
			}
...
errLine = "826";
			for(i=tempOldLedgercounter;i<oldLedgerRows;i++)
			{
				...
				pstmt_p = conp.prepareStatement(query);
				pstmt_p.setInt(1,transaction_id);
				pstmt_p.setInt(2,tempCompany_Id);
				pstmt_p.setInt(3,testvoucher_id);
	...
	errLine = "855";
				...
				pstmt_p.setString (21,""+ledgerPercent[i]);
	errLine = "458";
				int a525 = pstmt_p.executeUpdate();
				pstmt_p.close();
	errLine = "861";
				transaction_id++;
	errLine = "869";
				//System.out.println("<br> Insert new Ledger in FT"+i);
			 }
			}//end of for

		}
//END OF INSERT DATA..FOR NEW LEDHER IN FT..
//System.out.println("Before the commit data");
conp.commit();
dataCommitted = true;
	}//if
//conp.commit();

String tempQuery="select Receive_no,StockTransfer_Type from receive where active =1 and receive_id="+receive_id;
pstmt_g=cong.prepareStatement(tempQuery);
rs_g=pstmt_g.executeQuery();
String Receive_no="";
...
while(rs_g.next())
{
	...
	StockTransfer_Type=rs_g.getString("StockTransfer_Type");
...
{
	response.sendRedirect("../Master/NewEditPurchaseForm.jsp?command=pedit&receive_id="+receive_id+"&Iv_id=0&Fv_id=0&message=save&Receive_no="+Receive_no);
}
else
	{
	response.sendRedirect("../Inventory/InvStockTransferDetail_New.jsp?Receive_Id="+receive_id+"&StockTransfer_Type="+StockTransfer_Type+"&command=Default&mesage=Default");
	}
}//try
catch(Exception e)
{
	...
	C.returnConnection(cong);
	C.returnConnection(conp);
	out.print("<br>841 Bug in NewEditpurchaseUpdate.jsp at ErrorLine"+errLine+" exception is:"+e);
}%>