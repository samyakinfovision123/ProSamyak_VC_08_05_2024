...
<%=A.getMasterArrayAll(cong,"Companyparty","party_id","",company_id)%> </td></tr>
	...
	<input type=submit value='Edit Sale Return' name=command class='Button1' >
</td>
</tr>
</table>
</form>			
...
if("Edit Sale Return".equals(command))
	{
//cong=C.getConnection();
...
{
query="Select * from Receive  where Receive_Date between ? and ? and Company_id=? and Purchase=1  and Receive_Sell=1 and Active=1 and R_Return=1 and Opening_Stock=0  and ProActive=0 order by Receive_Date,Receive_No,Receive_Id";
}
//receive=0 specifies that the receive is consignment receive
else{
query="Select * from Receive  where Receive_Date between ? and ? and Company_id=?  and Receive_FromId="+party_id+" and Purchase=1 and Receive_Sell=1 and Active=1 and R_Return=1 and Opening_Stock=0  order by Receive_Date,Receive_No,Receive_Id";
}
...
<td>
	<%=i++%></td>
<td><A href= "EditSaleReturnForm.jsp?command=pedit&receive_id=<%=Receive_id%>" target=_blank>
	<%=rs_g.getString("Receive_no")%></a></td>

<td>&nbsp;<%=A.getNameCondition(cong,"Voucher","ref_No","where Voucher_No='"+Receive_id+"'")%></td>
...
<td>
<%=i++%></td>
<td><A href= "EditSaleReturnForm.jsp?command=pedit&receive_id=<%=Receive_id%>" target=_blank><%=rs_g.getString("Receive_no")%></a></td>
...
<tr><td align=right colspan=8><b>Total Purchase</b></td>
<td align=right></td>
