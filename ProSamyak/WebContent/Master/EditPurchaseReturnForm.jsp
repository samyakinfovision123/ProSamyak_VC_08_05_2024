...
//out.print("80 receive_id"+receive_id);
	while(rs_g.next())
		{
...
//out.print("<br>185 receivetransaction_id[n]"+receivetransaction_id[n]);
...
//out.print("<br> 217 Consignment_ReceiveId"+Consignment_ReceiveId);
		...
		while(rs_g.next())
			{
				...
				//out.print("<br>217 newConsignment_ReceiveId[i]"+newConsignment_ReceiveId[i]);
				nn++;
			}//while
			pstmt_g.close();
	}
		...
		while(rs_g.next())
			{
				...
				amount_tocompare[i] = str.mathformat((amount_tocompare[i] + quantity[i]),d);
...
query="Select * from Receive_Transaction where Receive_Id=? and Active=1";
pstmt_g = cong.prepareStatement(query);
	pstmt_g.setString(1,Consignment_ReceiveId); 
	rs_g = pstmt_g.executeQuery();	
	int nn=0;
	while(rs_g.next())
		{
			amount_tocompare[nn] = rs_g.getDouble("Available_Quantity"); 
			out.print("207 amount_tocompare[nn]"+amount_tocompare[nn]);
			nn++;
		}//while
*/
pstmt_g.close();

discount_amt = str.mathformat((subtotal * (discount/100)),d);
//out.println("<font color=red>discount_amt:</font>"+discount_amt);
temp_amt = str.mathformat((subtotal - discount_amt),d);
...
//out.print("<br>222 testvoucher_id "+testvoucher_id);
String ctax_id= A.getNameCondition(conp,"Ledger","ledger_id","Where Ledger_Name='C. Tax' and Company_id="+company_id+"" ); 
//out.print("<br>224 ctax_id "+ctax_id);
//out.print("<br>225 receive_id "+receive_id);

int ledgers= Integer.parseInt(A.getNameCondition(conp,"Voucher","ToBy_Nos","Where Voucher_Type=10 and Voucher_No='"+receive_id+"'"));
//out.print("<br>228 ledgers "+ledgers+"<br>228 receive_id "+receive_id);
...
//---------------------------------------------------------
var discount_amt=0;
if((document.mainform.discount_amt.value=="")){
discount_amt= (subtotal*document.mainform.discount.value)/100;

temp_dis = new String(discount_amt);
var i_dis=temp_dis.indexOf(".") ;
var temp1_dis="";
if(i_dis>0)
	{
i_dis =i_dis + d +1 ;
temp1_dis=temp_dis.substring(0,i_dis);
discount_amt=temp1_dis;
	}

document.mainform.discount_amt.value=discount_amt;

}
if(document.mainform.discount.value=="0"){
discount_amt=document.mainform.discount.value;
document.mainform.discount.value=((document.mainform.discount_amt.value)*100)/subtotal;
}
if(document.mainform.discount_amt.value != (((subtotal*document.mainform.discount.value)/100)))
{
if(document.mainform.discount_amt.value > (((subtotal*document.mainform.discount.value)/100)))
	{
discount_amt=document.mainform.discount_amt.value;
document.mainform.discount.value=((document.mainform.discount_amt.value)*100)/subtotal;
...
var temp_total= parseFloat(document.mainform.subtotal.value)-parseFloat(document.mainform.discount_amt.value)+parseFloat(subtotal1);

//alert("392 temp_total "+temp_total);
var tax_amt= 0;
...
var finaltotal= parseFloat(document.mainform.subtotal.value)-parseFloat(document.mainform.discount_amt.value)+parseFloat(document.mainform.ctax_amt.value)+parseFloat(subtotal1);
...
<FORM name=mainform
action="EditPurchaseReturnForm.jsp" method=post onSubmit='return onLocalSubmitValidate()'>
...
<tr>
<th colspan=9 align=center>Edit Purchase Return </th>
</tr>
<tr>
<td>No
	<input type=text name=consignment_no size=5 value="<%=receive_no%>" onBlur='return nullvalidation(this)'></td>
<td>Ref No<input type=text name=ref_no size=10 maxlength=10 value="<%=ref_no%>"></td>
...
{
condition="Where Super=0 and Purchase=1 and active=1";
}
%>
...
<td>	<%=AC.getMasterArrayCondition(conp,"PurchaseSaleGroup","purchasesalegroup_id",purchasesalegroup_id,"where Active=1 and PurchaseSaleGroup_Type=1",company_id)%></td>
...
<input type=text name='stockdate' size=8 maxlength=10 value="<%=format.format(stockdate)%>" onblur='return  fnCheckMultiDate(this,"Due Date")'>
</td>
<td colspan=2>
<script language='JavaScript'>
...
<td colspan=1>Due Days<input type=text name=duedays size=1 value="<%=duedays%>" onBlur="validate(this)" style="text-align:right"> <input type=hidden name="duedaysold" value="<%=duedays%>"></td>
...
%>
		<td COLSPAN=2> 
		<%=A.getNameCondition(conp,"Master_Location","Location_Name","where Location_Id ="+location_id[0])%></td>
<%
	}
	else
	{
...
<td>
	...
	<input type=text name=quantity<%=i%> value="<%=quantity[i]%>" size=5 OnBlur='return quantity(this)' style="text-align:right">
</td>
<td><input type=text name=rate<%=i%> value="<%=rate[i]%>" size=5  style="text-align:right" <%=rate_rd%>></td>
...
if(ledgers!=0)
{
query="Select * from Financial_Transaction FT , Voucher V where V.Voucher_Id=FT.Voucher_Id and V.Voucher_type=10 and V.voucher_No='"+receive_id+"' and FT.Active=1 and Ledger_Id > "+ctax_id+" order by Tranasaction_Id";
pstmt_g = cong.prepareStatement(query);
//pstmt_g.setString(1,testvoucher_id); 
rs_g = pstmt_g.executeQuery();	
...
<td><input type=text name=ctax_amt size=8 OnBlur='return calcTotal(this)' style="text-align:right" value="<%=str.mathformat(""+((subtotal-(discount*subtotal)/100)*ctax)/100,d)%>"></td>
<td colspan=2></td>
</tr>

<tr>
<td colspan=1></td>
<!--<td colspan=4>--><td>Total in <%=local_currencysymbol%></td>
<td colspan=1><input type=text readonly name=total size=8 value="<%=str.mathformat(""+total,d)%>" style="text-align:right" OnBlur='return calcTotal(this)'><input type=hidden name=old_total size=8 value="<%=str.mathformat(""+total,d)%>"></td>
</tr>
...
//out.print("<br>868 lotledger "+lotledger);
String add_lots="";
int newlots=0;
String add_ledgers="";
int newledgers=0;
...
//out.print("<br> old_lots "+old_lots);

String ledgers = request.getParameter("ledgers");
//out.print("<br>896 ledgers "+ledgers);
...
//newlotid[i]=A.getNameCondition("Lot","Lot_Id","where Lot_No="+newlotno[i]);
...
*/
//---------------------------------------------------------------
var temp_total= parseFloat(document.mainform.subtotal.value)+parseFloat(subtotal1);
var tax_amt= 0;
if((document.mainform.ctax_amt.value=="")){
tax_amt=
(temp_total*document.mainform.ctax.value)/100;
...
//var finaltotal=temp_total+tax_amt;
var finaltotal= parseFloat(document.mainform.subtotal.value)+parseFloat(document.mainform.ctax_amt.value)+parseFloat(subtotal1);
 temp_tot= new String(finaltotal);
var i_tot=temp_tot.indexOf(".") ;
var temp1_tot="";
if(i_tot>0)
...
<tr>
<th colspan=8 align=center>Edit Purchase Return </th>
</tr>

<tr>
<td>No
<!--<td colspan=1>--> <input type=text name=consignment_no size=5 value="<%=consignment_no%>" onBlur='return nullvalidation(this)'></td>
<td>Ref No<input type=text name=ref_no size=10 value="<%=ref_no%>"></td>
...
<td>Purchase Group</td>
	<td align=left>
	<%=AC.getMasterArrayCondition(conp,"PurchaseSaleGroup","purchasesalegroup_id",A.getNameCondition(conp,"Master_PurchaseSaleGroup","purchasesalegroup_id","where PurchaseSaleGroup_Name='Normal' and PurchaseSaleGroup_Type=1 and company_id="+company_id),"where Active=1 and PurchaseSaleGroup_Type=1",company_id)%></td>
...
onchange='stdate();' onfocus='stdate();' ></td>

<td colspan=2>
<script language='JavaScript'>
if (!document.layers) {document.write("<input type=button class='button1' onclick='popUpCalendar(this, mainform.stockdate, \"dd/mm/yyyy\")' value='Stock Date' style='font-size:11px ; width:100'>")}
</script>	
<input type=text name='stockdate' size=8 maxlength=10 value="<%=stockdate%>"></td>
...
<td colspan=8>Due Days<input type=text name=duedays size=1 value="<%=duedays%>" onBlur="validate(this)" style="text-align:right"><input type=hidden name="duedaysold" value="<%=duedaysold%>"> </td>
</tr>
...
<td>Location</td>
<td>
<%=A.getMasterArrayCondition(conp,"Location","masterlocation",newlocation_id[0],"where company_id="+company_id)%>
	<%//=A.getName("Location",newlocation_id[0])%></td>
<%}%>

<td colspan=2>Sales Person 
<%=A.getMasterArrayCondition(conp,"SalesPerson","salesperson_id",salesperson_id,"where Active=1 and company_id="+company_id+"") %>
</td>
<%
if("0".equals(currency_id))
{
...
<input type=hidden name=lotno<%=i%> value="<%=lotno[i]%>"><input type=text name=newlotno<%=i%> id=newlotno<%=i%> autocomplete=off value="<%=newlotno[i]%>"  size=10 <%=temp1%>  onfocus='handleFocus("newlotno<%=i%>")'> </td>
<input type=hidden name=location_id<%=i%> value="<%=location_id[i]%>">
<input type=hidden name=newlocation_id<%=i%> value="<%=newlocation_id[i]%>"></td>
<td><input type=text name=pcs<%=i%> value="<%=pcs[i]%>" onBlur="validate(this)" size=2 style="text-align:right" <%=temp1%>></td>
...
<td><input type=text name=amount<%=i%> value="<%=amount[i]%>" size=8 style="text-align:right" OnBlur='return calcTotal(this)' <%=temp1%>></td>
<td><input type=text name=remarks<%=i%> value="<%=remarks[i]%>" size=8 <%=temp1%>></td>
</tr>
<script language="javascript">
	var lobj<%=i%> = new  actb(document.getElementById('newlotno<%=i%>'), lotNoArray);
</script>	
...
<!--<td>Narration<input type=text name=description value="<%=description%>"></td> -->
...
out.println("<font color=red> FileName : EditSaleForm.jsp command=sedit<br>Bug No Samyak1031 : "+ Samyak1031);}
%>
...
*/	
//---------------------
//newlotid[i]=A.getNameCondition("Lot","Lot_Id","where Lot_No='"+newlotno[i]+"' and company_id="+company_id);
pcs[i]=""+request.getParameter("pcs"+i);
old_quantity[i]=""+request.getParameter("old_quantity"+i);
quantity[i]=""+request.getParameter("quantity"+i);
...
<FORM name=mainform
action="EditPurchaseReturnUpdate.jsp?" method=post  >
...
<%=consignment_no%></td>
 ...
 <td colspan=2>Ref No:
 <input type=hidden name=ref_no size=10 value="<%=ref_no%>">
<%=ref_no%></td>
...
onblur='return  fnCheckMultiDate1(this,"Due Date")' tabindex=1>
</td>
</tr>
...
<td>Purchase Group :
	<%=A.getNameCondition(conp,"Master_PurchaseSaleGroup","purchasesalegroup_Name","where Active=1 and PurchaseSaleGroup_Type=1 and Purchasesalegroup_id="+purchasesalegroup_id+ " and  company_id= "+company_id)%>
...
<td align=right>Quantity<br>(Carat)</td>
...
<%}%>
<td align="right"><%=str.format(""+rate[i],d)%></td>
...
<td colspan=1 align="right"><%=str.format(""+total_newqty,3)%></td>
<td colspan=2>&nbsp;</td>
...
<td colspan=2>&nbsp;</td>
<td colspan=3>Tax (%)</td>
<td colspan=1 align="right"><input type=hidden name=ctax size=8 OnBlur='return calcTotal(this)' value="<%=ctax%>" style="text-align:right"> <%=str.format(""+ctax,d)%></td>
...
<td colspan=2>&nbsp;</td>
<td colspan=4>Total in <%=local_currencysymbol%></td>
<td colspan=1 align="right"><input type=hidden readonly name=total size=8 value="<%=total_amount%>" style="text-align:right"> <%=str.format(""+total_amount,d)%><input type=hidden name=old_total  value="<%=old_total%>" > </td>
<td colspan=3>&nbsp;</td>
</tr>
</tr>
<tr>
<!--<td colspan=1></td>-->
<!--<td colspan=2  align=right>Difference Account in <%=local_currencysymbol%></td>-->
<!--<td colspan=2 align=right></td>
<td colspan=2 align=right><B><%=str.format(difference,d)%>--><input type=hidden readonly name=difference size=10 value="0"><!--</td>-->
<input type=hidden name="diffdebit" value="-1">
<% if("1".equals(diffdebit))
	{%>
...
<tr>
<td colspan=2>&nbsp;</td>
	<td colspan=4>Due Days<input type=hidden name=finalduedays size=1 value="<%=finalduedays%>" onBlur="validate(this)" style="text-align:right"> <%=finalduedays%></td>
<td colspan=3>&nbsp;</td></tr>

<tr>
	<td colspan=6>Narration :<input type=hidden name=description size=75 value="<%=description%>"><%=description%></td>

</tr>
<td colspan=9 align=center>
<input type=button name=command value=BACK onClick='history.go(-1)'  class='button1' tabindex=3> &nbsp; &nbsp;&nbsp;<input type=submit name=command value=UPDATE class='button1' tabindex=2> </td>
</tr>
</TABLE>
</td>
</tr>
