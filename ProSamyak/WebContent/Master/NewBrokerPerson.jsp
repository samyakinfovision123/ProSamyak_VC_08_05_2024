...
<script language=javascript src="../Samyak/BrokerPurchasePerson.js"></script>
...
<script language="javascript">
	...
	<%
		errLine="72";
			String SalesPersonArray = "";
			String SalesPersonId = "";
			String SalesPersonQuery = "Select SalesPerson_Name,SalesPerson_Id from Master_SalesPerson where Active in(1,0) and Company_Id="+company_id+" and purchasesale=2 order by SalesPerson_Name";
		errLine="76";		
			...
			while(rs_g.next()) 	
			{
				if(rs_g.isLast())
				{
					SalesPersonId += "\"" +rs_g.getString("SalesPerson_Id")+"\"";
					SalesPersonArray += "\"" +rs_g.getString("SalesPerson_Name")+"\"";
			...
			pstmt_g.close();
		errLine="223";	
			out.print("var SalesPersonArray=new Array("+SalesPersonArray+");");
...
function editCancel(no)
{
	if(no==1)
	{
		...
		var str="../Master/SalesPerson.jsp?command=Default&message=Broker";
	...
	if(no==2)
	{
		...
		//var str="../Master/EditBrokerPurchasePerson.jsp?command=Default&salesperson_id="+document.new_mainform.cancel_receiveid.value;
		var str="../Master/EditSalePurchaseBrokerPerson.jsp?command=Default&salepurchasebroker_id=2&salesperson_id="+document.new_mainform.cancel_receiveid.value;
	...
	if(no==3)
	{
		...
		var str="../Master/EditSalesPerson.jsp?command=edit&message=Broker&brokerview=yes";
...
{
	last_button_pressed="2"; 
	getNextBrokerPurchasePerson_header(document.new_mainform.cancel_invno.value,'<%=company_id%>','Edit',document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
	return false;
} 
function check()
{
...
{	
	...
	getNextBrokerPurchasePerson_header(document.new_mainform.cancel_invno.value,'<%=company_id%>',s,document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
...
var SalesPersonobj = new  actb(document.getElementById('cancel_invno'), SalesPersonArray);
...
{ out.println("<br><font color=red> FileName : NewBrokerPerson.jsp Bug No Samyak499 : "+ Samyak499+"Error on"+errLine);
}
%>
</center>
</form>
