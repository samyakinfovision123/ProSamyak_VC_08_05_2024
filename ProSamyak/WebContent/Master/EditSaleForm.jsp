...
String tempv=""+A.getNameCondition(conp,"Voucher","Voucher_Id"," where Voucher_No='"+receive_id+"'");
String ref_no=""+A.getNameCondition(conp,"Voucher","Ref_No"," where Voucher_Id="+tempv);
//out.print("<br>ref_no=" +ref_no);
String description=""+A.getNameCondition(conp,"Voucher","Description"," where Voucher_Id="+tempv);
...
Inv_id=A.getNameCondition(conp,"Voucher","Voucher_Id","where Voucher_No='"+receive_id+"' and Voucher_Type=1 and Active=1");
//out.print("<br>85 receive_id "+receive_id);
//out.print("<br>86 Inv_id "+Inv_id);

Finv_id=A.getNameCondition(conp,"Voucher","Voucher_Id","where Referance_VoucherId="+receive_id+" and Voucher_Type=8 and Active=1");
...
//out.println("<BR> 184"+company_query);
rs_g = pstmt_p.executeQuery();
	while(rs_g.next()) 	
	{
...
//out.print("<br>228 ledgers "+ledgers);
...
<body bgColor=#ffffee onLoad='onload_form(this);return finalTotal()' background="../Buttons/BGCOLOR.JPG">
<FORM name=mainform
action="EditSaleForm.jsp" method=post onSubmit='return onLocalSubmitValidate()'>
...
<tr>
	<td>Sale Type</td>
	<td><%=A.getCashAccounts(conp,"cash_id",FTcash_id,company_id,"1")%></td>
...
<tr>
<td>Sales Person </td>
<td><%=A.getMasterArrayCondition(conp,"SalesPerson","salesperson_id",salesperson_id,"where Active=1 and company_id="+company_id+"") %>
</td>
...
<td>Rate / Unit</td>
<td>Amount in <input type=text name=currency_text size=3 readonly value=<%=local_currencysymbol_Show%>></td>
<td>Remarks</td>
</tr>
<% 
for (int i=0;i<counter;i++)
...
<td>
<input type=text name=newlotno<%=i%> value="<%=lot_no[i]%>" size=10 onBlur='return nullvalidation(this)'  onfocus='handleFocus("newlotno<%=i%>")'> </td>
...
<td><input type=text name=rate<%=i%> value="<%=rate[i]%>" size=5  style="text-align:right" onblur='return calcTotal(this)'></td>
<td align=left><input type=text name=amount<%=i%> value="<%=str.mathformat(""+amount[i],d)%>" size=8 style="text-align:right" OnBlur='return calcTotal(this)'></td>
...
{%>
<tr>
<td align=left><%=i+1%><input align=left type="checkbox" name="findeleted<%=i%>" value="yes"> </td>
...
<td colspan=1 align=left><input type=text name=subtotal size=8 readonly value="<%=str.mathformat(""+subtotal,d)%>" style="text-align:right"></td>
<td colspan=2></td>
</tr>
<tr>
<!-- <td colspan=1></td> -->
...
<td align=left><input type=text name=ctax_amt size=8 OnBlur='return finalTotal()' style="text-align:right" value="<%=str.mathformat(""+((subtotal-(discount*subtotal)/100)*ctax)/100,d)%>"></td>
<td colspan=2></td>
</tr>
...
<td colspan=1 align=left><input type=text readonly name=total size=8 value="<%=str.mathformat(""+total,d)%>" style="text-align:right" OnBlur='return calcTotal(this)'></td>
</tr>
...
var flag=onLocalSubmitValidate();
//alert(flag);
if(flag)
	{
<%
...
function onLocalSubmitValidate()
	...
	calcTotal(document.mainform.amount<%=i%>)
...
<% if( (Iv_id>0 && Fv_id>0) || pdcount==0 )
	{%>
<td>Sale Type</td>
<td><%=AC.getCashAccounts(conp,"cash_id",cash_id,company_id,"1")%></td>
<input type=hidden name="Inv_id" value="<%=Iv_id%>">
<input type=hidden name="Finv_id" value="<%=Fv_id%>">
...
<td>
<%=AC.getMasterArrayCondition(conp,"SalesPerson","salesperson_id",salesperson_id,"where Active=1 and company_id="+company_id+"") %>
</td>
<%
if("0".equals(currency_id))
{
...
<input type=text name=newlotno<%=i%> value="<%=newlotno[i]%>"  size=10 <%=temp1%>  onfocus='handleFocus("newlotno<%=i%>")'> 
</td>
...
<td align=left><input type=text name=amount<%=i%> value="<%=amount[i]%>" size=8 style="text-align:right" OnBlur='return calcTotal(this)' <%=temp1%>></td>
...
<input type=text name=newlotno<%=i%> size=10 onBlur='return nullvalidation(this)' value="0"  onfocus='handleFocus("newlotno<%=i%>")'>
</td>
...
<td><%=i+1%><input type="hidden" name="org_transacionid<%=i%>" value="noid">
	...
	<input type="hidden" name="findeleted<%=i%>" value="no"> </td>
<td colspan=3 align=left><%=A.getArray(conp,"Ledger","ledger"+i,"",company_id+" and yearend_id="+yearend_id ,"Expense") %> &nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<td colspan=3>C.Tax (%)</td>
<td colspan=1 align=left><input type=text name=ctax size=8  value="<%=ctax%>" style="text-align:right"></td>
<td align=left><input type=text name=ctax_amt size=8 OnBlur='return finalTotal()' style="text-align:right" value="0"></td>
<td colspan=2></td>
</tr>
<tr>
<!-- <td colspan=1></td> -->
<td colspan=4>Total in <%=local_currencysymbol%></td>
<td colspan=1 align=left><input type=text readonly name=total size=8 value="<%=total%>" style="text-align:right" OnBlur='return calcTotal(this)' ></td>
</tr>
...
<tr>
<td colspan=1>&nbsp;</td>
<!--<td colspan=1  align=left>Difference Account in <%=local_currencysymbol%></td>
<td colspan=3 align=right>&nbsp;</td>
<td colspan=1 align=right><B><%=str.format(difference,d)%></B>-->
