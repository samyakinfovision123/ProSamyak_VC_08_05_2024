...
//out.print("<br>154 sreceive_Id="+sreceive_Id);
String dreceive_Id=""+(Integer.parseInt(sreceive_Id)+1);
errLine="167";
//out.print("<br>157 dreceive_Id="+dreceive_Id);
...
//out.print("<br>163 description="+description);
errLine="177";
...
//	out.print("<br> 163 Except Gain");
	query="select ReceiveTransaction_Id,Lot_Id, Quantity, Local_Price,Dollar_Price,Location_Id,Remarks from Receive_Transaction where Receive_Id="+sreceive_Id+" order by Lot_SrNo";
...
errLine="241";	samount[s]=Double.parseDouble(str.mathformat(""+(sqty[s]*srate[s]),d) );
	...
	stotaldollaramount+=sdollaramount[s];
			...
			sremark[s]=rs_g.getString("Remarks");
			//out.print("<br>252 sremark[s]="+sremark[s]);
			s++;
		}
	pstmt_p.close();
...
//	out.print("<br> 163 Except Loss");
	...
	query="select ReceiveTransaction_Id,Lot_Id, Quantity, Local_Price,Dollar_Price,Location_Id,Remarks from Receive_Transaction where Receive_Id="+dreceive_Id+" order by Lot_SrNo";
	errLine="253";
	...
	dtotaldollaramount+=ddollaramount[ds];
			...
			dremark[ds]=rs_p.getString("Remarks");
			//out.print("<br>280 dremark[ds]="+dremark[ds]);
			ds++;
		}
		errLine="275";
	pstmt_g.close();
 ...
 {%>
	...
	<body bgColor=#ffffee  onLoad="document.mainform.stocktransfer_no.focus();" background="../Buttons/BGCOLOR.JPG">
	<FORM name=mainform action="EditStock_InOutForm.jsp" method=post>
...
<tr>
<th colspan=11>Destination Lots</th>
</tr>
<tr>
	<td>Sr. No.</td>
	<td>Lot No.</td> 
...
<tr>
	<td><%=i+1%></td>
	<td><input type=text name="dlotno<%=i%>" value="<%=A.getNameCondition(conp,"Lot","Lot_No","where Lot_Id="+dlotid[i])%>" size=5  id="dlotno<%=i%>" onblur="getDescSize('<%=company_id%>', document.mainform.dlotno<%=i%>.value, document.mainform.datevalue.value, 'dlotid<%=i%>','ddescriptionname<%=i%>', 'dsizename<%=i%>', 'drate<%=i%>', 'ddescriptionname<%=i%>', 'purchase' );" autocomplete=off onfocus="this.select();">
	<input type=hidden name="dlotid<%=i%>" value="" id="dlotid<%=i%>"
	></td>
     
	<input type=hidden name="ddescriptionname<%=i%>" value="" size=5 style="text-align:left" id="ddescriptionname<%=i%>" autocomplete=off onfocus="this.select();"></td>
	<input type=hidden name="dsizename<%=i%>" value="" size=5 style="text-align:left" id="dsizename<%=i%>" onfocus="this.select();"></td>
	...
	<input type=hidden name=to_location<%=i%> value=<%=A.getNameCondition(conp,"Master_Location","location_id","where location_name='Main' and company_id="+company_id)%>>
	...
	<td><input type=text name="slotno<%=i%>" id="slotno<%=i%>" value="<%=A.getNameCondition(conp,"Lot","Lot_No","where Lot_Id="+slotid[i])%>" size=5  onblur="return calcAmounts1('<%=i%>');" autocomplete=off onfocus="this.select();">
	<input type=hidden name="slotid<%=i%>" id="slotid<%=i%>" value="0" size=5>
	</td>
	
	<td><input type=text name="dremark<%=i%>" value="<%=dremark[i]%>" style="text-align:right" size=5  onfocus="this.select();">
...
<%
	...
	<input type=hidden name="sremark<%=i%>" value="<%=sremark[i]%>" style="text-align:right" size=5 >
...
<table align=center borderColor=Skyblue border=1 WIDTH="85%">
<tr>
<td colspan=10>Narration
<input type=text size=100 name=description value="<%=description%>" onfocus="this.select();"></td>
</tr>
<tr>
	<!-- <td  align=right>Add Source Lots : --> 
	<!-- <input type=hidden name="saddlots" value="0" size=4 style="text-align:right" OnBlur="setSourceLot(this);" onfocus="this.select();">  -->
	
	<!-- <td  align=right>Add Dest Lots :  -->
	 <!-- <input type=hidden name="daddlots" value="1" size=4 style="text-align:right" onfocus="this.select();">  -->

	
<!-- <td align=center><input type="submit" name="command" value="Add" class="button1" onmouseover="this.className='button1_over';" onmouseout="this.className='button1';">
 -->
...
try
	...
	//out.print("<br>1045 sreceive_Id="+sreceive_Id);
	//out.print("<br>1046 dreceive_Id="+dreceive_Id);
	...
	//out.print("<br> 1025 sold_lots"+sold_lots);
	//out.print("<br> 1025 sold_lots"+t);
	...
	//out.print("<br> 1030 Tempdold_lots="+Tempdold_lots);
	int dold_lots=Integer.parseInt(request.getParameter("dold_lots"));
	//System.out.println("<br>1025 daddlots="+daddlots);
	//System.out.println("<br>1026 sold_lots="+sold_lots);
	...
	function calcAmounts(rowNum)
	{
		...
		document.mainform.dtotalqty.value=temp_QtTotal.toFixed(3);
		//document.mainform.sqty0.value=temp_QtTotal.toFixed(3);
		//alert("document.mainform.dtotalqty.value="+document.mainform.dtotalqty.value);	
	...
	<body bgColor=#ffffee  onLoad="document.mainform.stocktransfer_no.focus();setCurrency()" background="../Buttons/BGCOLOR.JPG">
...
<TABLE borderColor=Skyblue align=center border=1 WIDTH="85%" cellspacing=0 cellpadding=2 >
 ...
 -->
	...
	<% if(dcounter>0)
		...
		%>
	<tr>
	<th colspan=9>Destination Lots</th>
	</tr>
	<tr>
		<td>Sr. No.</td>
		<td>Lot No.</td>
	...
	String to_location=request.getParameter("to_location"+i);
	String slotno=request.getParameter("slotno"+i);	
	...
	<tr> 
		<td><%=i+1%></td>
		<td><input type=text name="dlotno<%=i%>" value="<%=dlotno%>" size=5 id="dlotno<%=i%>" onblur="getDescSize('<%=company_id%>', document.mainform.dlotno<%=i%>.value, document.mainform.datevalue.value, 'dlotid<%=i%>','ddescriptionname<%=i%>', 'dsizename<%=i%>', 'drate<%=i%>', 'ddescriptionname<%=i%>', 'purchase' );" autocomplete=off onfocus="this.select();">
		<input type=hidden name="dlotid<%=i%>" value="<%=dlotid%>" id="dlotid<%=i%>"></td>
		...
		<td>
		<input type=text name="slotno<%=i%>" value="<%=slotno%>" size=5 id="slotno<%=i%>" onblur="return calcAmounts1(<%=i%>)" autocomplete=off onfocus="this.select();">
		<input type=hidden name="slotid<%=i%>" value="" size=5 id="slotid<%=i%>" >
		</td>
	...
	<tr>
		<td><%=i+1%></td>
		<td><input type=text name="dlotno<%=i%>" value="#" size=5 onblur="getDescSize('<%=company_id%>', document.mainform.dlotno<%=i%>.value, document.mainform.datevalue.value, 'dlotid<%=i%>','ddescriptionname<%=i%>', 'dsizename<%=i%>', 'drate<%=i%>', 'ddescriptionname<%=i%>', 'purchase' );" autocomplete=off onfocus="this.select();">
		<input type=hidden name="dlotid<%=i%>" value=""></td>
		...
		value=<%=A.getNameCondition(conp,"Master_Location","location_id","where location_name='Main' and company_id="+company_id)%>>
		<td>
		...
		<input type=text name="slotno<%=i%>" value="#" size=5 id="slotno<%=i%>"  autocomplete=off onfocus="this.select();">
		<input type=hidden name="slotid<%=i%>" value="" size=5 id="slotid<%=i%>" >
		</td>
...
<FORM name=mainform
action="EditStock_InOutUpdate.jsp" method="post">
	...
	<tr>
	 ...
	 for(int i=0;i<scounter;i++)
		...
		<tr>
			...
			<input type=hidden name="sReceiveTransaction_Id<%=i%>" value="<%=sReceiveTransaction_Id[i]%>" 	>
			<%//=A.getNameCondition(conp,"Master_Location","Location_Name","where Location_id="+from_location[i])%>
			<input type=hidden name=from_location<%=i%> value="<%=from_location[i]%>">
		...
		<td><%=sremark[i]%>
			<input type=hidden name="sremark<%=i%>" value="<%=sremark[i]%>" style="text-align:right">
...
<% if(dcounter>0)
		...
		{%>
			<tr>
				...
				<input type=hidden name="dReceiveTransaction_Id<%=i%>" value="<%=dReceiveTransaction_Id[i]%>" 	>
				<%//=A.getNameCondition(conp,"Master_Location","Location_Name","where Location_id="+to_location[i])%>
				<input type=hidden name=to_location<%=i%> value="<%=to_location[i]%>">
