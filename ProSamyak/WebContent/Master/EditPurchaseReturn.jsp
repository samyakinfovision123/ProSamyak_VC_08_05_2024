...
<%=A.getMasterArrayAll(cong,"Companyparty","party_id","",company_id)%> </td></tr>
	...
	<input type=submit value='Purchase Return' name=command  class="button2">
</td>
</tr>
</table>
</form>			
...
if("Purchase Return".equals(command))
	{
...
{
	//out.print("inside if");
query="Select * from Receive  where Receive_Date between ? and ? and Company_id=? and Purchase=1  and Receive_Sell=0 and Active=1 and R_Return=1 and ProActive=0  order by Receive_Date,Receive_No,Receive_Id";
}
//receive=0 specifies that the receive is consignment receive
else{
	//out.print("inside else party_id");
query="Select * from Receive  where Receive_Date between ? and ? and Company_id=?  and Receive_FromId="+party_id+" and Purchase=1 and Receive_Sell=0 and Active=1 and R_Return=1 and ProActive=0  order by Receive_Date,Receive_No,Receive_Id";
}
...
<td>
	<%=i++%></td>
<td><A href= "EditPurchaseReturnForm.jsp?command=sedit&receive_id=<%=Receive_id%>" target=_blank>
	<%=rs_g.getString("Receive_no")%></a></td>
<td><%=A.getName(cong,"CompanyParty",receive_from)%>  </td>
...
<td>
<%=i++%></td>
<td><A href= "EditPurchaseReturnForm.jsp?command=sedit&receive_id=<%=Receive_id%>" target=_blank><%=rs_g.getString("Receive_no")%></a></td>
<td><%=A.getName(cong,"CompanyParty",receive_from)%>  </td>
<td bgcolor=<%=dispaly_bgcolor%>><%=display_sale%></td>
<td><%=format.format(rs_g.getDate("Receive_Date"))%></td>
<td><%=rs_g.getString("Receive_Lots")%></td>
...
<tr><td align=right colspan=7><b>Total Sale</b></td>
<td align=right></td>
