...
//out.println("main_account"+main_account);
...
String account_name	=request.getParameter("companyparty_name");
String account_no=request.getParameter("account_no");
String bank_id= request.getParameter("bank_id");
System.out.print("<br>bank_id=" +bank_id);
...
double opening_balance = Double.parseDouble(request.getParameter("ledger_amount"));
//out.println("<br>102 exchange_rate is"+opening_balance);
String credit_debit=request.getParameter("main_ledger_amount");
...
String selectquery ="Select Account_Name from Master_Account where Account_name ='"+account_name+"' and company_id="+company_id;
...
selectquery ="Select CompanyParty_Name from Master_CompanyParty where CompanyParty_Name ='"+account_name+"' and company_id="+company_id;
pstmt_p = conp.prepareStatement(selectquery);
rs_g = pstmt_p.executeQuery();	
while(rs_g.next()){i++;}
pstmt_p.close();
...
response.sendRedirect("NewCompanyPartyAccount.jsp?command=Default&message= "+temp+"<font color=blue> "+account_name+" </font>successfully Added&type="+type+"&account_no="+account_no);
...
<center><font class='msgcolor1' >Account Name <%=account_name%> already exists.<BR><input type=button name=command value='BACK' onClick='history.go(-1)' class='button1'></center>
<%
}//if end
...
if("1".equals(accounttype_id))
{
	temp_account_no="Bank";
}
else
{
	temp_account_no="Cash";
}
response.sendRedirect("NewCompanyPartyAccount.jsp?command=Default&message= "+bankparty_id+"<font color=blue> "+account_name+" </font>successfully Updated&type="+bankparty_id+"&account_no="+temp_account_no);
