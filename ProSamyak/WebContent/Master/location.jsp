...
<script language=javascript src="../Samyak/Location.js"></script>
...
<html>
<body>
<body bgcolor=ffffee onLoad="init()" background="../Buttons/BGCOLOR.JPG">
...
try{
 errLine="40";
	...
	String locationid= request.getParameter("cancel_receiveid");
...
%>
	...
	<%
		errLine="201";
			...
			String LocationId = "";
			String LocationQuery = "Select Location_Name,Location_Id from Master_Location where Active in(1,0) and Company_Id="+company_id+" order by Location_Name";
		errLine="204";		
			pstmt_g = cong.prepareStatement(LocationQuery, ResultSet.TYPE_SCROLL_INSENSITIVE,                   ResultSet.CONCUR_READ_ONLY);
			...
			while(rs_g.next()) 	
			{
				if(rs_g.isLast())
				{
					LocationId += "\"" +rs_g.getString("Location_Id")+"\"";
					LocationArray += "\"" +rs_g.getString("Location_Name")+"\"";
			...
			pstmt_g.close();
		errLine="223";	
			out.print("var LocationArray=new Array("+LocationArray+");");
...
function editCancel(no)
{
	...
	if(no==1)
	{
		...
		var str="../Master/NewLocation.jsp?command=Default";
	...
	if(no==2)
	{
		...
		//var locid=document.new_mainform.cancel_receiveid.value;
	
		var str="EditLocation.jsp?command=Next&location_id="+document.new_mainform.cancel_receiveid.value;
		window.open(str,"bottom", ["Top=100","Left=0","Toolbar=no", "Location=0","Menubar=yes","Height=600","Width=1200", "Resizable=yes","Scrollbars=yes","status=no"]);
	...
	if(no==3)
	{
		...
		var str="../Master/EditLocation.jsp?command=edit&view=yes";
...
function checkDateValidity(ele)
{
	...
	if(document.new_mainform.invoice_selection[1].checked)
	{
		checkMe(ele);
	}
	else
	{
		var str_invno=document.new_mainform.cancel_invno.value;
		document.new_mainform.cancel_invno.value=str_invno.toUpperCase(); 
	}
} //checkMe()
function checkDate()
{
	getNextLocation_header(document.new_mainform.cancel_invno.value,'<%=company_id%>',"Edit",document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
...
function setValue()
{
	if(document.new_mainform.invoice_selection[0].checked)
	{
		document.new_mainform.cancel_invno.value="<%=lastInvoice_No%>";
//		document.new_mainform.cancel_invno.value="P-";
	...
	if(document.new_mainform.invoice_selection[1].checked)
	{
		...
		//var temp="<%=format.format(inv_date1)%>";
		var temp="<%=format.format(lastInvoice_Date)%>";
		document.new_mainform.cancel_invno.value=temp;
		document.new_mainform.cancel_invno.focus();
	}
} //setValue
...
{	
	...
	getNextLocation_header(document.new_mainform.cancel_invno.value,'<%=company_id%>',s,document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
...
<tr bgcolor=#FFDCB9>
	<td>
		<input type=button name=command style='width:80' value="NEW" class='button1'  onmouseover="this.className='button1_over';" onmouseout="this.className='button1';" onClick="editCancel('1')">
	</td>
	...
	<td>
		...
		<input type='text' name='cancel_invno' onfocus="this.select();" value="" maxlength='10' size='15'  id=cancel_invno  autocomplete=off OnBlur="checkDate();">
...
var Locationobj = new  actb(document.getElementById('cancel_invno'), LocationArray);
...
%>
	<td >&nbsp;&nbsp;&nbsp;&nbsp;
		<input type=button name=command style='width:80' value="EDIT" class='button1'  onmouseover="this.className='button1_over';" onmouseout="this.className='button1';" onClick="editCancel('2')">
	</td>
<%
	}
	else
...
%>
	<td >&nbsp;&nbsp;&nbsp;&nbsp;
		<input type=button name=command style='width:80' value="EDIT" class='button1'  onmouseover="this.className='button1_over';" onmouseout="this.className='button1';" onClick="editCancel('2')" disabled>
	</td>
...
%>
	 ...
	 <td >&nbsp;&nbsp;&nbsp;&nbsp;
		<input type=button style='width:80' name=CANCEL value="VIEW" class='button1'  onmouseover="this.className='button1_over';" onmouseout="this.className='button1';" onClick="editCancel('3')" >
	</td>
	<td>
	...
	<!-- <td>
		<input type=button style='width:60' name=PRINT value="PRINT" class='button1'  onmouseover="this.className='button1_over';" onmouseout="this.className='button1';" Onclick='editCancel(4);'>
	</td>	
 -->
 </tr>
</table> 
...
{ out.println("<br><font color=red> FileName : Location.jsp Bug No Samyak499 : "+ Samyak499+"Error on"+errLine);
}
%>
</center>
</form>
