...
// out.println("<br>Command is "+command);

String voucher_type=""+request.getParameter("voucher_type");
String from_india=""+request.getParameter("from_india");
//out.print(" <br>voucher_type "+voucher_type);
String costheadsubgroup_id= ""+request.getParameter("costheadsubgroup_id"); 
...
//out.print("<br>type_toby[counter]"+type_toby[counter]);
//for (int i=0;i<=count_actual;i++)
//{
...
<form action="ADDUpdateVoucher.jsp?message=Default" method=post name=mainform >
...
<%=I.getCostHeadArray(conp,conp,"costheadsubgroup_id",costheadsubgroup_id,company_id,"") %></td>
...
<tr>
<td colspan=1>To </td>
<td><%=A.getMasterArrayAccount(conp,"account_id"+i,account_id[i],company_id+" and yearend_id="+yearend_id,"Normal") %> 
</td>
<td></td>
<td align=center>
...
<td colspan=1>By </td>
<td><%=A.getArray(conp,"Ledger","account_id"+i,account_id[i],company_id+" and yearend_id="+yearend_id,"Ledger")%>
</td><td align=center>
...
%>
<tr>
<input type=hidden name=srno<%=counter%> value=<%=count_actual%>>
<input type=hidden name=type_toby<%=counter%> value=to>
<td colspan=1>To </td><td> 
<%=A.getMasterArrayAccount(conp,"account_id"+counter,"",company_id+" and yearend_id="+yearend_id,"Normal") %>
</td>
<td></td>
<td align=center>
<input type=text size=8 name=amount<%=counter%>  value="0" OnBlur='return calcTotal(this)' style="text-align:right">
</td>
<td><input type=text size=8 name=remarks<%=counter%>  value="" > </td>
</tr>
<%	}
...
<td colspan=1>By </td>
<td><%=A.getArray(conp,"Ledger","account_id"+count_actual,"",company_id+" and yearend_id="+yearend_id,"Ledger")%></td>
...
<input type=text size=8 name=debit_total  readonly value="" style="background:#FFFF99" style="text-align:right">
</td>
<td align=center bgcolor=#FFFF99><input type=text size=8 name=credit_total  readonly value="" style="background:#FFFF99" style="text-align:right"></td>
<td></td>
</tr>
...
out.println("<br><font color=red> FileName : Payment.jsp Bug No Samyak444 : "+ Samyak444);
}
}//if ADD
//------End Add here --------------------------------------------
...
<%
	String account_id1=""+request.getParameter("account_id0");
	String from_india=""+request.getParameter("from_india");
...
/*Currency check removed on clients request 24/09/2005
...
//out.print("<br>VoucherNo->"+voucher_no);
String type= ""+request.getParameter("voucher_type");
String costheadsubgroup_id= ""+request.getParameter("costheadsubgroup_id"); 
String costheadgroup_id= ""+A.getNameCondition(conp,"Master_CostHeadSubGroup","CostHeadGroup_Id","where CostHeadSubGroup_Id="+costheadsubgroup_id+""); 
...
pstmt_p.close();
/*out.print("**********");
out.print("v_old"+v_old);
out.print("v_id"+v_id);
out.print("no_exist"+no_exist);
*/
...
java.sql.Date date_value=new java.sql.Date((YY-1900),(MM-1),DD);
...
query="Update Voucher set  Company_Id=?, Voucher_No=?, Voucher_Date='"+date_value+"', ToBy_Nos=?,  Voucher_Currency=?, Exchange_Rate=?, Voucher_Total=?, Local_Total=?, Dollar_Total=? , Description=?, Modified_By=?, Modified_On='"+D+"', Modified_MachineName=?, CostHeadGroup_Id=?, CostHeadSubGroup_Id=?,Ref_No=?  where Voucher_Id=?";
...
pstmt_p.setString (12,""+costheadgroup_id);	
pstmt_p.setString (13,""+costheadsubgroup_id);
...
{
//	out.print("<br>*******i--->"+i);
query="Update Financial_Transaction set  Company_Id=?, Voucher_id=?,Sr_No=?, For_Head=?, For_HeadId=?, Description=?, Transaction_Type=?, Amount=?, Local_Amount=?, Dollar_Amount=?, Modified_By=?, Modified_On='"+D+"', Modified_MachineName=?, Ledger_Id=?, Transaction_Date='"+date_value+"',  CostHeadGroup_Id=?, CostHeadSubGroup_Id=? where Tranasaction_Id=?";
...
pstmt_p.setString (14,""+costheadgroup_id);	
pstmt_p.setString (15,""+costheadsubgroup_id);
}

pstmt_p.setString (16,""+ft_id[i]);		 
