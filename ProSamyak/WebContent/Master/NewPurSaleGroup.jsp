...
<script language=javascript src="../Samyak/SalePurchaseGroupPerson.js"></script>
...
try{
 errLine="12";
	...
	<%
		errLine="72";
				String PurchaseSaleGroupArray = "";
				String PurchaseSaleGroupId = "";
				
				errLine="76";
			
				String SalesPersonQuery ="select PurchaseSaleGroup_Name,PurchaseSaleGroup_Id from Master_PurchaseSaleGroup where  company_id= "+company_id+" and Active in(1,0)and PurchaseSaleGroup_Type="+purchasesalegroup_type+" order  by  PurchaseSaleGroup_Name";
			
			pstmt_g = cong.prepareStatement(SalesPersonQuery, ResultSet.TYPE_SCROLL_INSENSITIVE,                   ResultSet.CONCUR_READ_ONLY);
			...
			while(rs_g.next()) 	
			{
				if(rs_g.isLast())
				{
					PurchaseSaleGroupId += "\"" +rs_g.getString("PurchaseSaleGroup_Id")+"\"";
					PurchaseSaleGroupArray += "\"" +rs_g.getString("PurchaseSaleGroup_Name")+"\"";
			...
			pstmt_g.close();
		errLine="223";	
			out.print("var PurchaseSaleGroupArray=new Array("+PurchaseSaleGroupArray+");");
...
function editCancel(no)
{
	if(no==1)
	{
		...
		var str="../Finance/PurchaseSaleGroupType.jsp?command=Default&message=Default&purchasesalegroup_type=<%=purchasesalegroup_type%>";
	...
	if(no==2)
	{
		...
		var str="../Master/EditPurchaseSaleGroup.jsp?command=Edit1&message=Default&purchasesalegroup_type=<%=purchasesalegroup_type%>&purchasesalegroup_id="+document.new_mainform.cancel_receiveid.value;
	...
	if(no==3)
	{
		...
		var str="../Master/EditPurchaseSaleGroup.jsp?command=Edit&purchasesalegroup_type=<%=purchasesalegroup_type%>&message=Default&view=yes";
...
{
	last_button_pressed="2"; 
	getNextSalePurchaseGroupPerson_header(document.new_mainform.cancel_invno.value,'<%=company_id%>','<%=purchasesalegroup_type%>','Edit',document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
	return false;
} 
function check()
{
...
{	
	...
	getNextSalePurchaseGroupPerson_header(document.new_mainform.cancel_invno.value,'<%=company_id%>','<%=purchasesalegroup_type%>',s,document.new_mainform.cancel_invno,document.new_mainform.cancel_receiveid,last_button_pressed);
...
var PurchaseSaleGroupobj = new  actb(document.getElementById('cancel_invno'), PurchaseSaleGroupArray);
...
{ out.println("<br><font color=red> FileName : NewPurSaleGroup.jsp Bug No Samyak499 : "+ Samyak499+"Error on"+errLine);
}
%>
</center>
</form>
