...
<%	}
	...
	query="Select * from Receive R, Receive_Transaction RT,Lot L where R.Receive_id=RT.Receive_Id and  R.Opening_Stock=1 and   RT.Lot_Id=L.Lot_Id and L.Lot_No=? and L.company_id=?";
...
//		out.print("<br> PLocation_Id "+PLocation_Id[j]);
	...
	query="select * from Master_Location where  Active=1 and company_id="+company_id+" and location_id NOT IN (Select Location_Id from Receive R, Receive_Transaction RT where R.Receive_id=RT.Receive_Id and  R.Opening_Stock=1 and   RT.Lot_Id=ALL (select Lot_Id from Lot where Lot_No =? and company_id=?) )";
...
%>
	...
	<body bgcolor=ffffee background="../Buttons/BGCOLOR.JPG">
	<form action="UpdateOpeningStock.jsp?" method=post name=f1>
...
<!-- 					<td width='15%'><%=Lot_No%></td>
 -->					<td width='20%' align=left><%=A.getName(cong,"Location",""+ALocation_Id[k])%><input type=hidden name=ALocation_Id<%=k%> value='<%=ALocation_Id[k]%>'></td>
					<td width='15%' align=left><%=A.getName(cong,"Unit",A.getNameCondition(conp,"Lot","Unit_Id","where Lot_No='"+Lot_No+"' and Company_Id="+company_id+""))%><input type=hidden name=AUnit_Id<%=k%> value='<%=A.getNameCondition(cong,"Lot","Unit_Id","where Lot_No=\""+Lot_No+"\" and Company_Id="+company_id+"")%>'></td>
...
<%	
	...
	response.sendRedirect("OpeningStock.jsp?command=Default&message=<center><font color=red>Lot No. </font><font color=blue>"+fromLot_No+"</font><font color=blue> or </font><font color=blue>"+toLot_No+"</font><font color=blue> does not exist</font>" );
...
<html>
	...
	<table borderColor=skyblue align=center border=1 cellspacing=0 cellpadding=2 width='90%' rules=no>
		...
		<% if(!(n==0))
			...
			<td bgcolor=skyblue width='80%' align=center><b>Opening Stock for <%=A.getNameCondition(cong,"Master_Location","Location_Name","where Location_Id="+Location_Id)%></b></td>
		</tr>
		<%	}else{	%>
				<tr>
				<td bgcolor=skyblue width='100%' align=center colspan=2><b>Opening Stock for <%=A.getNameCondition(cong,"Master_Location","Location_Name","where Location_Id="+Location_Id)%></b></td>
		</tr>
		<%	}	%>
...
<html>
	...
	<table borderColor=skyblue align=center border=1 cellspacing=0 cellpadding=2 width='90%'>
		...
		<%	}	%>
			...
			<td bgcolor=skyblue align=center width='80%'><b>Opening Stock for <%=A.getNameCondition(cong,"Master_Location","Location_Name","where Location_Id="+Location_Id)%></b></td>
		</tr>
		<tr>
			<td colspan=2>
			<table borderColor=skyblue align=center border=1 cellspacing=0 cellpadding=2 width='100%'>
