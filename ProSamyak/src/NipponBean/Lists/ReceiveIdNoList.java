...
public class ReceiveIdNoList {
 ...
 java.sql.Date receive_Date = new java.sql.Date(System.currentTimeMillis());
   ...
   * gets all the lots
   */
  public void getIdNumber(String company_id,String receive_no,String isNext) {
	  ...
	  try{
		...
		if(!(isNoDate))
		{
			...
			if("SaleNext".equals(isNext))
			{
				//dd1=dd1+1;
				query="select receive_id,receive_date from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1 and  receive_date > '"+InputDate+"' order by receive_date";
			}
			if("SalePrev".equals(isNext))
			{
				//dd1=dd1-1;
                query="select receive_id,receive_date from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1 and  receive_date < '"+InputDate+"' order by receive_date desc";			
			}
			...
			if("SaleFirst".equals(isNext))
			{
				//dd1=1;
				//mm1=0;
			    query="select receive_id,receive_date from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1 order by receive_date";
			}
			if("SaleLast".equals(isNext))
			{
				//dd1=31;
				//mm1=11;
			    query="select receive_id,receive_date from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1 order by receive_date desc";
			}
			...
			rs_g.close();
			pstmt_g.close();
			//receive_no1[0]=""+format.format(D4);
		...
		if("SaleNext".equals(isNext))
		{
			query="select receive_id,receive_no from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1 and receive_id >( select receive_id from Receive where company_id="+company_id+" and receive_no like '"+receive_no+"') order by receive_id";
		}
		if("SalePrev".equals(isNext))
		{
		...
		if("SaleFirst".equals(isNext))
		{
			query="select receive_id,receive_no from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1 "; 
		}
		if("SaleLast".equals(isNext))
		{
			query="select receive_id,receive_no from Receive where company_id="+company_id+" and receive_sell=0 and purchase=1 and R_Return=0 and Opening_Stock=0 and stocktransfer_type=0 and active=1  order by receive_id desc";
		}
