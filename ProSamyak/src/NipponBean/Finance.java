...
public String ClosingBalance(Connection con,String ledger_id,java.sql.Date D1,int d) 
{
try{
String query="";
// cong=C.getConnection();

 query="Select * from Ledger where  Ledger_id="+ledger_id+" ";
pstmt_g = con.prepareStatement(query);
rs_g = pstmt_g.executeQuery();	
...
query="Select * from Financial_Transaction where  Ledger_id="+ledger_id+" and Transaction_Date <= ? and Active=1 order by Transaction_Date,Tranasaction_Id ";
pstmt_g = con.prepareStatement(query);
pstmt_g.setString(1,""+D1);
rs_g = pstmt_g.executeQuery();	
...
public String ClosingBankBalance(Connection con,String account_id,String company_id,java.sql.Date D1, int d) 
{
try{
 //cong=C.getConnection();
double opening_localbalance = 0;
double opening_dollarbalance = 0;
String query ="Select *from Master_Account where account_id="+account_id;
String account_no="";
String account_name="";
...
if("PN Account".equals(account_name))
		{
 query="Select * from Master_CompanyParty where company_id="+company_id+"  and Active=1 and PN=1";
pstmt_g = con.prepareStatement(query);
	rs_g = pstmt_g.executeQuery();	
	while(rs_g.next())
	{
opening_localbalance += str.mathformat(rs_g.getDouble("Opening_PNLocalBalance"),d);
opening_dollarbalance += str.mathformat(rs_g.getDouble("Opening_PNDollarBalance"),2);
...
query="Select *from Financial_Transaction where For_Head=1 and For_HeadId="+account_id+" and Transaction_Date<=? and Active=1 order by Transaction_Date, Voucher_id";
...
public String ClosingBankBalanceDollar(Connection con,String account_id,String company_id,java.sql.Date D1, int d) 
{
try{
 //cong=C.getConnection();
double opening_localbalance = 0;
