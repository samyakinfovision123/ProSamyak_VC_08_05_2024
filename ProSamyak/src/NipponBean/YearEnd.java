...
if(rs_g.getBoolean("Receive_Sell"))//purchase
{
	...
	//System.out.println("PURCHASE_GAINLOSS="+PURCHASE_GAINLOSS);
}
else //sale
	{
...
public String YearEndClosingDebtorsCreditors(Connection con,java.sql.Date D1, String company_id,int d, String yearend_id)
{		
try{
String query="";
// cong=c.getConnection();
...
/****************    ****************/
public String YearEndClosingDebtorsCreditorsLocation(Connection con,java.sql.Date D1, String company_id,int d, String yearend_id,String salesPerson_Id)
{		
try{
String query="";
errLine="432";
// cong=c.getConnection();
 query="Select * from Master_CompanyParty where company_id=?  and Active=1 and salesPerson_Id=? order by Transaction_Currency, CompanyParty_Name ";
pstmt_g = con.prepareStatement(query);
pstmt_g.setString(1,company_id);
pstmt_g.setString(2,salesPerson_Id);
...
return ""+Samyak109+"errLine="+errLine;
	}
//finally{c.returnConnection(cong); }
...
/**********       *****************/
public String YearEndClosingDebtorsCreditorsLocationDollar(Connection con,java.sql.Date D1, String company_id,int d, String yearend_id,String salesPerson_Id,String party_group_id)
{		
try{
String query="";
errLine="432";
// cong=c.getConnection();
 query="Select * from Master_CompanyParty as MCP,Ledger as L where MCP.company_id="+company_id+" and L.company_id=MCP.company_id and MCP.Active=1 and L.Active=1 and MCP.salesPerson_Id="+salesPerson_Id+" and L.for_head=14 and L.partyGroup_Id="+party_group_id+" and MCP.companyParty_id=L.for_headId order by MCP.Transaction_Currency, MCP.CompanyParty_Name ";
...
while(rs_g.next())
{
	voucher_type[j]=rs_g.getInt("Voucher_Type");
	String r_id=rs_g.getString("Voucher_No");
	Local_Total[j]= str.mathformat(rs_g.getDouble("Dollar_Total"),d);
...
String temp_ex= ""+partyexchangeGainLoss(con,""+party_id[i], company_id,D1);
StringTokenizer tempe = new StringTokenizer(temp_ex,"/");
purchase_gainloss[i] =0;//str.mathformat(Double.parseDouble( (String)tempe.nextElement()),d);
 sale_gainloss[i] = 0;//str.mathformat(Double.parseDouble( (String)tempe.nextElement()),d);
...
for(i=0; i<partycounter; i++)
	{
 purchase[i]=total_purchase[i];
 slid[i]=A.getNameCondition(con,"Ledger","Ledger_id","Where For_head=14 and For_headid="+party_id[i]+" and Ledger_type=1"+" and PartyGroup_Id="+party_group_id);
//System.out.println("Slid["+i+"]="+slid[i]);
 plid[i]=A.getNameCondition(con,"Ledger","Ledger_id","Where For_head=14 and For_headid="+party_id[i]+" and Ledger_type=2");
...
return tempr;
	...
	{//c.returnConnection(cong);
System.out.println("Exception "+Samyak109+" errLine="+errLine);
return ""+Samyak109+"errLine="+errLine;
	}
//finally{c.returnConnection(cong); }
...
//method calculating the profit loss
public double getNetProfitLoss(Connection cong, int d, java.sql.Date D1, java.sql.Date D2, String company_id, String yearend_id){
...
query="Select * from Receive R, Payment_Details PD  where R.Receive_Id=PD.For_HeadId  and R.Receive_Date between ? and ? and R.Company_id=? and R.Purchase=1 and R.Opening_Stock=0 and R.Active=1 and R.R_Return=0 and R.Receive_CurrencyId=0 order by R.Receive_id";
...
double purchase_local_currency =0;
double sale_local_currency =0;
double purchase_LC =0;
// purchase local currency gain or loss
double sale_LC =0;
// sale local currency gain or loss
//for individual while loop for same receive id 
double t_purchase_LC =0; 
// purchase local currency gain or loss
double t_sale_LC =0;
// sale local currency gain or loss
...
category_opening[c]= YES.stockValue(cong,D1,company_id,"Opening",d, " and LotCategory_Id="+LotCategory_Id+"",yearend_id);
category_closing[c]= YES.stockValue(cong,D2,company_id,"Closing",d, " and LotCategory_Id="+LotCategory_Id+"",yearend_id);
...
double inv_total=str.mathformat(rs_g.getDouble("InvLocalTotal"),d);
...
String closing_amt1= YEF.ClosingBalance(cong,""+ledger_id[i],D2,d,yearend_id, company_id);
...
String closing_amt1= YEF.ClosingBalance(cong,""+ledgerid[i],D2,d,yearend_id,company_id);
...
String closing_amt1= YEF.ClosingBalance(cong,""+ic_ledgerid[i],D2,d,yearend_id,company_id);
...
net_profit =gross_profit - total_indirectrexpense + FINAL_GAINLOSS -total_indirectincome;
...
{
	System.out.println("Exception in getNetProfitLoss method");
}
return net_profit;
}// end getNetProfitLoss
